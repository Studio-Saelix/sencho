...
describe('registryReferenceDiscovery', () => {
  ...
  it('collects refs from a string build context equal to the stack root', () => {
    ...
    // Dockerfile must participate, not warn as outside the base.
    const warn = vi.spyOn(console, 'warn').mockImplementation(() => {});
    try {
      const scoped = discoverRegistryReferences(dir, {}, 'app');
      expect(scoped.referencedHosts).toEqual(['ghcr.io']);
  ...
  it('collects refs from an object build context equal to the stack root', () => {
    ...
    // must stay silent for the same reason the string form does.
    const warn = vi.spyOn(console, 'warn').mockImplementation(() => {});
    try {
      const scoped = discoverRegistryReferences(dir, {}, 'app');
      expect(scoped.referencedHosts).toEqual(['ghcr.io']);
  ...
  it('follows an explicit dockerfile key with the stack root as context', () => {
  ...
  it('still warns when the build context resolves strictly outside the base', () => {
  ...
  it('still warns when the explicit dockerfile key resolves outside the base', () => {
  ...
  it('warns as unreadable when the dockerfile key resolves to the stack root itself', () => {
