...
// render fail so the per-file fallback is what unscoped assertions exercise;
// the effective-path test overrides it to succeed.
...
describe('registryDeliverySelection', () => {
  ...
  it('service-scoped multi-file discovery uses the resolved compose file list', async () => {
    ...
    // The effective render merges both files: the override replaces web:1
    // with web:2, so the attested set is what the merged model fetches for
    // the service, not the per-file union (which would also include web:1).
    mockSpawn.mockImplementation(() => makeRenderProc(
      JSON.stringify({ services: { web: { image: 'ghcr.io/example/web:2' } } }),
      0,
    ));
    ...
    expect(refs.some(r => r.includes('web:1'))).toBe(false);
  ...
  it('restore-candidate recovery discovers the primary compose.yaml for a git multi-file stack', async () => {
    ...
    const discovery = await discoverRegistryReferences(stagingDir, {}, undefined, basis.composeFiles);
    const refs = discovery.referencedPullRefs;
