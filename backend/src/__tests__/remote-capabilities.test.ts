...
describe('probeRemoteCapability', () => {
  ...
  it('threads the abort signal into the remote meta probe', async () => {
    const controller = new AbortController();
    const spy = vi.spyOn(NodeRegistry.getInstance(), 'probeRemoteMeta')
      .mockResolvedValue({ kind: 'ok', meta: { version: '0.97.1', capabilities: ['fleet', CAP], ...ONLINE } });
    await probeRemoteCapability(NODE_ID, CAP, controller.signal);
    expect(spy).toHaveBeenCalledWith(NODE_ID, controller.signal);
  });
...
describe('probeRemoteMeta raw-kind classification', () => {
  ...
  it.each([
    ...
    ['EAI_AGAIN', 'other'],
    // axios uses ERR_CANCELED for an AbortController-triggered abort; the
    // fail-closed default keeps an aborted probe unreachable, never
    // unsupported, so a disconnect cannot downgrade a delivery into a plain
    // forwarded call.
    ['ERR_CANCELED', 'other'],
