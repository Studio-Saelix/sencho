...
describe('Stack lifecycle mutex', () => {
  it('returns 409 with stack_op_in_progress when a deploy is already running', async () => {
    const gate = deferred<{ recoveryId: string | null; deployedGenerationId: string | null; gitopsOperationId: string | null }>();
  ...
  it('releases the lock after a successful deploy so the next request acquires', async () => {
    mockDeployStack.mockResolvedValueOnce({ recoveryId: null, deployedGenerationId: null, gitopsOperationId: null });
