...
export type RegistryDeliveryProxyHopDecision =
  | { action: 'attempt'; probe: Extract<RemoteCapabilityProbe, { kind: 'supported' }> }
  | { action: 'skip' }
  | { action: 'aborted' };
...
export type RegistryDeliveryProxyGateResult =
  | { outcome: 'continue' }
  | { outcome: 'stop' }
  | { outcome: 'run-delivery'; probe: Extract<RemoteCapabilityProbe, { kind: 'supported' }> };
...
export async function decideRegistryDeliveryProxyHop(
  ...
  const probe = await wouldAttemptRegistryDelivery(
    nodeId,
    method,
    deliveryApiPath,
    req.registryDeliveryAbortController?.signal,
  );
  if (
    req.destroyed
    || req.aborted
  ...
  // A null probe means the route is not delivery-eligible at all, so skip and
  // let the request continue as a normal forwarded call.
  if (!probe) {
    return { action: 'skip' };
  }
  return probe.kind === 'supported' ? { action: 'attempt', probe } : { action: 'skip' };
}
