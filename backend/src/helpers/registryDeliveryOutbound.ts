...
function logDeliveryPassthrough(nodeId: number, probe: RemoteCapabilityProbe, stack: string | undefined): void {
  if (probe.kind === 'unsupported') {
    ...
    console.error('[registryDeliveryOutbound] REGISTRY_DELIVERY_TARGET_UNREACHABLE', { nodeId, stack, detail: probe.detail });
  }
}
...
export async function wouldAttemptRegistryDelivery(
  ...
  const probe = await probeRemoteCapability(nodeId, REMOTE_REGISTRY_EXACT_REF_PROOF_V1_CAPABILITY, abortSignal);
...
export async function augmentJsonBodyForRegistryDelivery(
  ...
  const probe = input.capabilityProbe
    ?? await probeRemoteCapability(input.nodeId, REMOTE_REGISTRY_EXACT_REF_PROOF_V1_CAPABILITY, input.abortSignal);
  if (probe.kind === 'unsupported' || probe.kind === 'unreachable') {
