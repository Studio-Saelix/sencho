...
import { hashPullRefList, hashSelectionInputs } from '../helpers/registryDeliveryHashes';
import { isValidStackName } from '../utils/validation';
import { resolveRegistryDeliverySelection } from '../helpers/registryDeliverySelection';
...
export class RegistryDeliveryService {
  ...
  async discoverOnTarget(request: RegistryDeliveryDiscoverRequest): Promise<RegistryDeliveryDiscoverResponse> {
    ...
    if (prepared) {
      ...
      const entry = store.getEntry(prepId);
      if (!entry) {
        throw new Error('Prepared source entry missing');
      }
      const selection = await resolveRegistryDeliverySelection({
        kind: entry.sourceKind,
        stackName: request.stack ?? request.stackName ?? '',
        nodeId,
        rootDir: payloadPath,
