 ...
 * start, update, rollback, backup, delete, git_apply, image_pull) per
 * (nodeId, stackName). A second request to the same stack while the first is
 * still running returns 409 instead of racing the first. Backup is included
 * because it rewrites the shared rollback slot, so it must not interleave with a
 * deploy/update/rollback on the same stack.
 * Git apply holds this lock across capture, promote, handoff, and optional deploy.
 * Image pull holds it because it writes the shared local image store that a
 * concurrent deploy would be resolving from.
 *
...
export type StackOpAction = 'deploy' | 'down' | 'restart' | 'stop' | 'start' | 'update' | 'rollback' | 'backup' | 'delete' | 'git_apply' | 'image_pull';
