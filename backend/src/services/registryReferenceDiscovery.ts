...
function readRegularFileSync(filePath: string, baseResolved: string): Buffer | null {
  // Unlike the build-path guards later in this file, the paths checked here
  // join a fixed filename onto a directory inside the base, so equality
  // with the base is unreachable and the strict form suffices.
...
function readFileSyncOrNull(resolved: string, baseResolved: string): Buffer | null {
  ...
  // check must accept equality, not just a strict descendant.
  // Silent here: the only caller already warns for the same condition, so
  // this guard is defense in depth only.
  if (!isPathWithinBase(resolved, baseResolved)) return null;
...
function collectServiceDockerfileRefs(
  ...
  // The stack root itself is in bounds: `build: .` resolves the context to
  // it, so only paths strictly outside the base reach this warn.
  if (!isPathWithinBase(contextPath, baseResolved)) {
    console.warn('[registryReferenceDiscovery] REGISTRY_DELIVERY_DOCKERFILE_OUTSIDE_BASE', {
      context: spec.context,
    });
    return;
  }
  if (!isPathWithinBase(dockerfilePath, baseResolved)) {
    console.warn('[registryReferenceDiscovery] REGISTRY_DELIVERY_DOCKERFILE_OUTSIDE_BASE', {
