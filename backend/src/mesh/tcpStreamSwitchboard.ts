...
/**
 * WS send-buffer level above which a tunnel stops reading local sockets and
 * reports backpressure to writers. One constant for both ends of a tunnel
 * (the pilot-side bridge and this switchboard) so the two cannot drift.
 */
export const TUNNEL_SEND_BUFFER_HIGH_WATER_MARK = 4 * 1024 * 1024;
...
export class TcpStreamSwitchboard {
    ...
    private isSaturated(): boolean {
        return this.ctx.ws.bufferedAmount > TUNNEL_SEND_BUFFER_HIGH_WATER_MARK;
    }
    ...
    private checkDrain(): void {
        // A closed tunnel can never drain, so resume everything it paused
        // rather than clearing the sets and stranding those sockets
        // mid-stream. Matches the bridge's close() path.
    ...
    public openReverseStream(target: { nodeId: number; stack: string; service: string; port: number }): ReverseTcpStreamHandle | null {
        ...
        // Every terminal path (tcp_close, idle timeout, rejection, tunnel
        // error, whole-switchboard cleanup) ends in _dispatchClose, so one
        // subscription here keeps a closed handle from lingering in the
        // awaiting-drain set and pinning its source socket.
        handle.once('close', () => { this.handlesAwaitingDrain.delete(handle); });
