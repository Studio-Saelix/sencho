...
export type ApiTokenValidation =
  ...
  | { ok: false; reason: 'not-api-token' | 'checksum' | 'not-found' | 'revoked' | 'expired' | 'owner-deleted' };

/**
 * Validate an API token with no side effects: format, checksum (timing-safe),
 * hash lookup, revocation, expiry, and owner existence. The format and checksum
 * checks short-circuit before the SQLite lookup so malformed or bad-checksum
 * keys never touch the database. Shared by the HTTP auth middleware, the
 * WebSocket upgrade handler, and the rate limiter's key generator so all three
 * agree on what counts as a live token.
 *
 * Owner deletion must invalidate tokens: SQLite foreign_keys is not enabled on
 * the main connection, so orphaned api_tokens rows can remain after DELETE FROM
 * users. Reject those here so HTTP and WS never treat a deleted user's
 * full-admin token as live (docs: "Owner deleted" → 401).
 */
export function validateApiToken(token: string): ApiTokenValidation {
  ...
  if (!db.getUserById(apiToken.user_id)) return { ok: false, reason: 'owner-deleted' };
