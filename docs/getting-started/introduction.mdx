...
<Frame>
  <img src="/images/introduction/home-overview.png" alt="Sencho Home view with the health masthead, resource gauges, the Stack health table, Recent alerts beside the Fleet heartbeat table, and the Configuration summary table." />
</Frame>
...
- **Stack health** lists stacks by state and load, with optional All nodes coverage across the inventory, uptime, CPU, memory, and a 10-minute CPU sparkline per stack.
- **Recent alerts** leads the operational row with the latest warnings, errors, and update notices, scoped to the active node or to every node. A row opens the stack, scan history, or Fleet update sheet it concerns, and **View all alerts** opens the full notification feed.
- **Fleet heartbeat** sits beside it on multi-node installs with every node's state, problem nodes first; a row opens that node's details in Fleet.
- **Configuration summary** closes the page with one row per domain (notifications, automation, security, recovery and alerts), expandable into each setting with the scope it applies to and a link to where it is managed.
