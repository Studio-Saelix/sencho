...
You land on **Home**, the default operational view. The health masthead reports **Healthy**, **Degraded**, or **Critical**, names the active node, shows how many nodes are registered, and lists any signals that need attention, alongside a running-container count, a live CPU/memory readout, and an alert count. The resource gauge strip tracks **CPU**, **Memory**, **Disk**, and **Network** with sparklines and threshold coloring. **Stack health** lists the active node's stacks, sorted by load by default (click a column header to sort by name, uptime, CPU, or memory instead), with each row's state, source (**Local** or **Git**), networks, port, uptime, CPU, memory, and a 10-minute CPU sparkline. When more than one node is registered, switch to **All nodes** for a progressive multi-node inventory.

<Frame>
  <img src="/images/quickstart/dashboard.png" alt="Sencho Home view with the health masthead, resource gauges with sparklines, the Stack health table, an operational row pairing Recent alerts with Fleet heartbeat, and the Configuration summary table." />
</Frame>

Below the stack table, the operational row leads with **Recent alerts**, with **Fleet heartbeat** beside it once remote nodes are registered. An alert row opens the stack, scan history, or Fleet updates sheet it points at, and a heartbeat row opens that node's details in Fleet. **Configuration summary** closes the page with one row per domain; expand a domain to see each setting, the scope it applies to, and a link to where it is managed.
