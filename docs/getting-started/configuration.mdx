...
Sencho reads the `Balloon:` field from `/proc/meminfo` when it is available and subtracts the ballooned amount from used, so the dashboard memory gauge, Fleet memory sorting, and host RAM alerts reflect the guest's real working set rather than the pre-balloon view. When `/proc/meminfo` is unreadable or the `Balloon:` field is absent, the behavior is unchanged.
