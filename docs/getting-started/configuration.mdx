...
## Nested LXC and Docker-in-LXC host metrics

Sencho reads host CPU and memory totals from the operating system view inside the container (`/proc` and related counters). On a correctly configured LXC, that view is the container's own capacity (for example 2 vCPU and 4 GiB). On a Proxmox or similar LXC **without** a virtualized `/proc` (typically provided by **lxcfs**), those same files can expose the hypervisor host instead, so Dashboard and Fleet gauges may show the outer host's core count and RAM.

Agent and primary Compose files stay unchanged for this case. Fix the host environment so `/proc` reflects the LXC:

1. Ensure **lxcfs** is installed and running on the Proxmox (or other) host that provides the LXC.
2. Confirm the LXC mounts lxcfs-backed `/proc` files (Proxmox does this when lxcfs is healthy).
3. Inside the LXC (and inside the Sencho or Pilot container), check that `/proc/meminfo` `MemTotal` and the visible CPU count match the LXC's configured limits, not the hypervisor.

When those checks pass, Sencho's existing host gauges and RAM alerts report the LXC capacity with no Compose or image special-casing.
