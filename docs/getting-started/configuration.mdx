...
## Host cgroup capacity (LXC and nested Docker)

When Sencho runs inside Docker on an LXC (or any host where `/proc` MemTotal and CPU lists do not match the node's cgroup limits), join the host cgroup namespace so Dashboard and Fleet can read finite `memory.max` / `cpu.max` values for that node.

In Compose, set `cgroup: host` on the Sencho (or pilot agent) service. For `docker run`, pass `--cgroupns=host`. The running service definition must include this setting; updating the image alone, including Pilot self-update against an existing Compose file, does not add it.

With host cgroup visibility, Sencho reports the strictest finite host-scoped memory and CPU limits above its own container leaf (so a Sencho `mem_limit` is not mistaken for node capacity). Memory used is a cache-aware working set (`memory.current` minus reclaimable inactive file cache). When the host cgroup namespace is not joined, or no finite host-scoped limit is visible, Sencho keeps the previous `/proc`-based totals with ZFS ARC and VM balloon adjustments.

CPU usage percentage and CPU host alerts continue to use the live load sample; only the core count and memory capacity path are cgroup-aware.
