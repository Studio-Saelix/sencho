...
Set `SENCHO_PUBLIC_URL` to the matching `https://` URL. For a private or self-signed issuer, also set `SENCHO_TLS_CA_FILE` so enrollment can give agents the CA bundle. Publicly trusted certificates do not need a CA file.
