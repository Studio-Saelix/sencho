...
| `SENCHO_MESH_PROXY_HEADERS` | *(unset)* | JSON object of extra HTTP headers central sends when it opens a mesh connection to a Distributed API Proxy node, for access gateways in front of the remote (for example Cloudflare Access service tokens). Key it by node name with `"*"` as the fallback for per-node values. Only sent when the node's API URL is HTTPS. See [Sencho Mesh](/features/sencho-mesh#reverse-proxies-tunnels-and-vpns). |
| `SENCHO_MESH_PROXY_CA_FILE` | *(unset)* | Path inside the container to a PEM bundle central trusts, in addition to the default roots, when it opens a mesh connection to a Distributed API Proxy node whose certificate comes from a private CA. See [Sencho Mesh](/features/sencho-mesh#reverse-proxies-tunnels-and-vpns). |
...
| `SENCHO_PUBLIC_URL` | *(request host)* | Set on the primary instance. Its externally reachable `http(s)://` URL, no trailing slash, baked into pilot enrollment so remote agents dial the public hostname rather than the address the admin used at setup. Use `https://` when this instance terminates TLS itself. |
| `SENCHO_TLS_CERT_FILE` | *(unset)* | Path inside the container to the PEM certificate (leaf plus any intermediates). Set together with `SENCHO_TLS_KEY_FILE` to listen with HTTPS. Unset keeps the HTTP listener, which is the right default behind a reverse proxy. |
| `SENCHO_TLS_KEY_FILE` | *(unset)* | Path inside the container to the PEM private key. Must be set with `SENCHO_TLS_CERT_FILE`; setting only one of the two refuses to start. |
| `SENCHO_TLS_CA_FILE` | *(unset)* | Optional CA bundle. When set, Pilot enrollment bind-mounts it into the agent as `SENCHO_PILOT_CA_FILE` and returns the PEM in the enrollment dialog. Required for private or self-signed issuers; leave unset for publicly trusted certificates. |
| `SENCHO_TLS_KEY_PASSPHRASE` | *(unset)* | Passphrase for an encrypted private key. Leave unset for unencrypted keys. |
...
## Native TLS on the control instance

When Sencho is not behind a TLS-terminating reverse proxy, point `SENCHO_TLS_CERT_FILE` and `SENCHO_TLS_KEY_FILE` at PEM files inside the container. Sencho then listens with HTTPS on port 1852. Pilot agents dial `wss://`, and private-registry credential delivery can use that confidential tunnel.

Both files must be set. Setting only one refuses to start rather than falling back to HTTP. Leave both unset when a reverse proxy already terminates TLS; that path keeps using `SENCHO_TRUSTED_PROXY_CIDRS`. These variables apply only to the control instance. On a Pilot agent (`SENCHO_MODE=pilot`) they are ignored, because the agent replays tunneled requests to its own loopback HTTP listener.

The certificate must include a Subject Alternative Name that matches how agents dial: a DNS name for a hostname, or an `iPAddress` SAN for a raw IP. A certificate with only a Common Name fails Node's verification.

Set `SENCHO_PUBLIC_URL` to the matching `https://` URL. For a private or self-signed issuer, also set `SENCHO_TLS_CA_FILE` so enrollment can give agents the CA bundle and the image health check can trust the issuer. Publicly trusted certificates do not need a CA file.

Existing Pilot agents keep their persisted `pilot.jwt` across a scheme change. Edit the agent compose file so `SENCHO_PRIMARY_URL` uses `https://` (and add the CA bind-mount if enrollment now includes one), then recreate the agent container.
...
services:
  sencho:
    ...
    environment:
      - COMPOSE_DIR=/opt/compose
      - SENCHO_PUBLIC_URL=https://192.168.1.50:1852
      - SENCHO_TLS_CERT_FILE=/app/certs/fullchain.pem
      - SENCHO_TLS_KEY_FILE=/app/certs/privkey.pem
      - SENCHO_TLS_CA_FILE=/app/certs/ca.pem
```
