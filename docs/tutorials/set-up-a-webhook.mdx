...
This tutorial covers creating and triggering one webhook for the **Deploy (down + up)** action. It doesn't cover the other five actions (Restart, Stop, Start, Pull & Update, Git source sync), native provider hooks on the Git Source sheet, writing CI YAML for a specific provider, or key rotation across many webhooks; see the [Webhooks](/features/webhooks) and [Git Sources](/features/git-sources) feature pages for the complete picture.
...
<Steps>
  ...
  <Step title="Stage a change for the webhook to apply">
    ...
    <Frame>
      <img src="/images/tutorials/set-up-a-webhook/webhook-save-only-menu.png" alt="Compose editor for storefront with the image line changed to nginx:1.28-alpine, and the dropdown next to Save & Deploy open with Save Only visible. The running container's logs below still show nginx/1.27.5, confirming the change is staged but not yet deployed." />
    </Frame>
  </Step>
...
<CardGroup cols={2}>
  ...
  <Card title="Git Sources" icon="code-branch" href="/features/git-sources">
    Provider hooks react to pushes from your Git host; the **Git source sync** webhook action is the CI-side equivalent when your pipeline already calls Sencho.
  </Card>
  ...
  <Card title="Deploy Compose Changes by Pulling From Git Instead of Editing by Hand" icon="code-branch" href="/tutorials/connect-a-git-source">
    Link a stack to a Git repository and run the same pull-review-apply cycle by hand from the Git Source sheet, no webhook required.
  </Card>
</CardGroup>
