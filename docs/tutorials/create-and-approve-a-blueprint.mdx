...
This tutorial does not cover stateful Blueprints (the extra confirmation step for named volumes and bind mounts), pinning a Blueprint to a specific node, moving a deployment between nodes, or Git-managed Blueprints (**Convert to Git**). **Apply now** in the steps below is the Inline path. See the [Blueprints](/features/blueprint-model) feature page for stateful workflows and Git-managed content, and [Move a Blueprint Deployment to a New Node](/tutorials/set-up-fleet-federation) once you've completed this one and want to relocate a deployment with Federation.
...
<Steps>
  ...
  <Step title="Author the Blueprint">
    ...
    Scroll down to **Selector** and leave **Labels** selected. Click the `edge` pill under **Match nodes with ANY of these labels**. The line below confirms `Resolves to nodes labelled any of [edge]`. Under **Drift policy**, leave **Suggest** selected, the middle option: Sencho will always detect drift on this Blueprint (including frozen image identity once a revision has been deployed), and Suggest additionally sends a notification when it happens, without redeploying anything on its own. (**Observe** stays silent about drift beyond the deployment row and Drift tab; **Enforce** auto-redeploys to restore a stopped container, or reapplies the frozen digest with `--pull never` when image identity disagrees.)
  ...
  <Step title="Review and confirm the rollout">
    ...
    Above the change list, the same dialog reads out the recorded GitOps authority and state: the three authority steps as chips, then the source and executable-artifact evidence, the placement and rollout state, and the per-target observations. An Inline Blueprint has no Git source or executable artifact set, so those two facets are omitted as not applicable, while placement and rollout report what is on record. A combined approval recorded as a single decision is labelled a legacy combined approval rather than one of the steps. The footer notes that confirming is bound to the evidence snapshot shown; if a recorded authority fact changes first, the apply is refused as stale and the dialog refreshes.

    <Frame caption="The rollout preview for the first deploy: one safe create, one informational warning, and the recorded authority and state for the application.">
      <img src="/images/tutorials/create-and-approve-a-blueprint/rollout-preview.png" alt="Confirm rollout dialog for whoami-edge reading Safe 1, Warnings 1, Blockers 0, legacy combined: pending. A GitOps authority and evidence block shows a rollout generation of none, a placement card reading 'blueprint bound', and a rollout card reading 'rollout not executable'. A Warnings section lists 'no persistent volumes detected'. A Changes section lists Local (local/online) · create · safe: New placement." />
    </Frame>
