...
## High GPU usage with full material effects
...
**Possible contributor:** Full material effects (glass blur on the top bar, sidebar, and floating surfaces) can cost more to composite on integrated graphics, single-board computers, and other constrained rendering devices. The reporter of this issue did not reproduce it when Windows selected the GPU or when Chrome used their discrete GPU. Fresh installs default to **Calm**, which already enables **Reduced effects**.

**What to try:**

1. Open **Settings → Appearance → Motion & effects**.
2. Turn on **Reduced effects**, or choose the **Calm** visual style (or **Readability mode**). That turns off glass blur and uses solid chrome fills.
3. On dual-GPU Windows PCs, you can also set the browser's GPU preference to let the OS decide, or prefer a discrete GPU, if your OS exposes that control.

**How to help validate a fix on your hardware:** After changing Appearance, leave the Sencho tab in the foreground on a quiet page for about 15 seconds, note browser GPU process utilization for about 10 seconds, then open an update dialog and compare. Close and reopen the tab between trials. On Windows, use Task Manager's GPU column for Chrome's GPU process. On Linux single-board devices, use whatever documented GPU or responsiveness counters your board provides, and label CPU-only readings as secondary. Report idle and dialog results with Reduced effects on versus off.
