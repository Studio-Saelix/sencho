...
Nothing is scored. Every state comes from evidence the hub or the node actually gathered, so a check that could not run reads as `unknown` or `unavailable` and never as healthy.

<Frame>
  <img src="/images/fleet-view/fleet-readiness-tab.png" alt="Fleet Readiness tab. The tab strip along the top lists Overview, Snapshots, Readiness, Map, Docker Labels, Deployments, Federation, Actions, and Secrets, with Readiness selected. The top card reads 'fleet readiness' over the headline 'Needs attention', with the sub-line '1 node needs attention' and a right-aligned 'last checked 3s ago' above '1 node'. Below it, a Findings card lists six rows. The first carries a NEEDS ATTENTION chip reading 'Stacks are not running' against 'Local / web', with an 'Open stack' button. The five below it carry an UNKNOWN chip: 'Policy sync state is unknown' with a Settings button, 'Stack readiness could not be determined' with a Snapshots button, 'No scan has completed yet' with a Security button, and 'Stack state is unknown' against 'Local / api' and 'Local / cache', each with an 'Open stack' button. The bottom matrix is one row for the single Local node, with a column per domain: Connectivity, Workloads, Updates, Recovery, Security, Control. Connectivity and Updates show a healthy dot, Workloads carries a NEEDS ATTENTION chip, and Recovery, Security, and Control each read UNKNOWN." />
</Frame>

### The summary

The card at the top names the fleet's worst state in a single word: `All clear`, `Partly unknown`, `Degraded`, `Unavailable`, or `Needs attention`. Under it, a sub-line counts how many nodes sit in each state, and a right-aligned `last checked` delta keeps ticking while the tab stays open, so you can tell a live reading from a stale one at a glance.

The order is deliberate and worst-first: a known problem outranks a condition the hub could not check, and a condition it could not check outranks a healthy one. A fleet where every node is fine does not outrank a fleet where one node is on fire because the other nodes reported first.

### Findings

**Findings** is the worklist. It lists every condition the fleet reported, problems and unknowns ahead of anything healthy, each row carrying the domain it came from, a plain-language line naming the node and stack it concerns, and a button that opens the surface where you act on it.

A finding that restates a stack's pre-update or rollback verdict repeats that verdict in the same words the stack's own readiness check uses, so `blocked` means the same thing in both places. Findings the current user cannot act on still appear; only the shortcut button is withheld.

### The readiness matrix

The matrix is one row per node, one column per domain, so a fleet-wide pattern (every node's security evidence is stale) is visible as a column rather than inferred from a stack of cards.
...
| **Workloads** | Stacks the node reports as crashed, exited, or only partly running, plus material uncertainty in the stack listing itself. |
| **Updates** | The canonical per-stack update verdict: which stacks are ready to update, which carry warnings, which need review first, and which are blocked. |
| **Recovery** | Whether rollback would actually save you for each stack, and whether a fleet snapshot covers it. |
| **Security** | Whether the node's scanner is available, how fresh its scans are, and whether its posture evidence was complete enough to trust. Security remains the surface for the detail. |
| **Control** | Policy Sync state for the node: in sync, failing, or paused because its control identity no longer matches. See [Fleet Sync](/features/fleet-sync). |

The **Control** column is admin-only.

A healthy cell is a dot rather than a word, so the eye lands on the cells that carry a state. A cell reads `needs attention`, `degraded`, `unavailable`, or `unknown` otherwise.

Clicking a node name opens that node's details.

### When a node cannot be reached

The tab is built so that one bad node degrades one row, not the page. A node keeps whatever the hub could establish about it and reports the rest as `unavailable` or `unknown` with the reason: it did not answer, its Pilot tunnel is down, it does not report a check yet, or the check did not finish inside its budget. Nodes that did answer render normally, and their results are never erased by a slower sibling.
