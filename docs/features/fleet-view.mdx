...
| **Status** | Either an `Up to date` success badge, an `Update` button when a newer release is available, a **Reapply configuration** action for Compose-managed nodes (including up-to-date rows), an in-progress / failed badge with retry and dismiss controls, or a `Skipped` badge when the version has been deferred. |
