...
3. **Run 41 deterministic rules** against the combination. Each rule is pure and produces zero or more findings with a severity, a message, and a suggested fix.
...
All 41 rules are listed below, organized by topic.
...
| Rule | Severity | What it detects |
|------|----------|----------------|
| Docker socket mounted | High | A service mounts `docker.sock` directly, granting broad control over the Docker host. |
| Docker socket proxy detected | Info | A dedicated Docker socket proxy service mounts `docker.sock` (known proxy image, name hint, or read-only socket with at least two proxy API group keys). Verify trust boundaries and enabled API groups. |
...
| Docker socket proxy allows mutating API access | Warning | A classified socket proxy enables `POST` and/or `DELETE` (value `1`) on the proxied Docker API. |
| Docker socket proxy on a non-internal network | Warning | A classified socket proxy attaches to at least one network that is not marked `internal`. |
| Docker API access routed through socket proxy | Note | A service does not mount `docker.sock`, shares a network with a classified proxy, and sets `DOCKER_HOST`. Coverage is treated as satisfied for All Clear; the note explains the topology. |
...
Five of the 41 rules cross-reference the stack's exposure intent and the access URLs documented in the Stack Dossier. These rules only fire when the stack publishes at least one host port.
