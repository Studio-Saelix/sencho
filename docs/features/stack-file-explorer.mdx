...
The file explorer is available on every Sencho tier. Write actions (upload, download, edit, create, rename, change permissions, delete) require **stack edit** permission on your account. Viewer accounts can browse the tree, preview text files, and inspect permissions in read-only mode.
...
<AccordionGroup>
  ...
  <Accordion title="Download button is missing">
    Confirm you are signed in to an account with **stack edit** permission. Viewer accounts can browse and preview but cannot download. To pull files from a viewer-only login, use a host terminal or `docker cp`.
  </Accordion>
