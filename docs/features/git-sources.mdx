...
<Note>
  Changing the repository, the branch, the compose files, the project directory, or the `.env` sync clears any staged commit, because the plan was built against the settings you just replaced. The state moves to **Reconcile required**; pull again to rebuild it. Changing only the token or the apply behavior leaves a staged commit alone, since neither changes what would be materialized.
</Note>

### When part of the state could not be proven

Under the state, the panel sometimes lists one or more things it could not prove. A managed manifest that names a different branch than the one configured now, an approval that could not be restored after a recovery, a record that has gone: each is reported as its own line.
