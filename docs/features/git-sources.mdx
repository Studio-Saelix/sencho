...
| **GitLab** | `X-Gitlab-Token` (shared secret) or `X-Gitlab-Hook-Signature-256` when present |
...
Delivery ids are taken from the host's idempotency headers when present (`X-GitHub-Delivery`, `X-Request-UUID`, and others) so retries dedupe cleanly.
