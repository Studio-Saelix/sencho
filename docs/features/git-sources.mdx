...
4. Click **Apply** to write the incoming files to disk. Apply stays disabled while local conflicts are present. Tick **Deploy after apply** in the same dialog to redeploy in one step.
...
- One row per classified operation: add, modify, remove, rename, or a local conflict. Unchanged files collapse to a count.
...
Sencho classifies every managed path against the incoming commit and the live disk.

- Matching content: applying writes files Sencho itself last wrote.
