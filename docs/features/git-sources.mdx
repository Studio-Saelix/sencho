...
Some teams store sensitive env files or file-backed secrets in Git as [SOPS](https://github.com/getsops/sops)-encrypted documents. Sencho keeps ciphertext in the repository and on disk. Decryption happens only in a short-lived overlay while validate, apply, deploy, or rollback runs Compose.
...
Sencho accepts age (X25519) recipients only. Documents that use PGP, cloud KMS, or other SOPS backends are refused with an unsupported-backend message.
...
<AccordionGroup>
  <Accordion title="Missing or rotated SOPS age key">
    A pull, apply, deploy, or rollback that needs SOPS decryption fails before Compose runs when this node does not hold a matching age identity. Open **Repository secrets**, confirm the readiness chip, and generate or import the key whose recipient appears in your SOPS files. After rotating a key, re-encrypt repository files with the new recipient before deleting the old identity. Rollback to a generation that still names a removed recipient stays blocked until you restore that identity or advance to a generation encrypted for a key you still hold.
  </Accordion>
