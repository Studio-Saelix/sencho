...
<AccordionGroup>
  ...
  <Accordion title="Repository too large">
    A single clone is capped to protect the host disk (100 MB by default). A compose repository is normally tiny, so hitting this usually means the tracked branch carries large binaries. Point the source at a repository or branch that holds just your compose and `.env` files. If a large repository is unavoidable, an operator can raise the ceiling with the `GITSOURCE_MAX_CLONE_BYTES` environment variable.
  </Accordion>
...
- **Clone size cap.** A single clone is bounded to protect the host disk, so very large repositories are rejected. Operators can adjust the ceiling with `GITSOURCE_MAX_CLONE_BYTES`.
