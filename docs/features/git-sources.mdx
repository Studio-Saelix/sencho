...
- **Complete project materialization.** Every repository-local input the project needs is materialized: the ordered compose files, implicit `compose.override.*` files, recursive `include:` and `extends.file` dependencies, service env files, file-backed configs and secrets, label files, and build contexts with `.dockerignore` semantics. The materialized set is recorded in a versioned managed-project manifest, and each pull stages a candidate that is validated with the exact deployment invocation before anything on disk changes.
- **Unsupported inputs are refused, not guessed.** Inputs that cannot be safely reproduced fail the pull with an actionable message: absolute or out-of-bound paths, URL includes, Git LFS pointers, submodule contents, symbolic links, and build contexts that exceed the size bounds. Nothing is applied until the declaration is fixed. Dynamic `\${VAR}` paths, host bind mounts, and external resources are never claimed as covered: they resolve at deploy time from the environment or the node, and the manifest records them as unmanaged.
...
- **Detach and export.** Removing a Git source renders the effective compose model into a single `compose.yaml`, keeps the materialized files, and removes Git tracking. Resolved environment values are baked into the exported file.
