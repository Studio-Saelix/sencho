...
The toolbar carries **Save** or **Update**, **Pull now** when a source is linked, **Suspend** or **Resume** when that action is offered, and **Remove** when a source is linked. Nested pull/apply and detach confirmations stay as separate dialogs on top of the sheet.
...
| **Suspended** | Reconciliation is paused until you resume. Polling, webhooks, and automatic apply stay off for this source. |
...
**Suspend** pauses reconciliation for one source: polling, webhooks, and automatic apply stop until you resume. Pull now and save stay available. An optional reason is stored with the suspension.
