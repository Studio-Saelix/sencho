...
<Frame>
  <img src="/images/git-sources/panel.png" alt="Git Source sheet opened from the stack editor, with crumb Stack, stack name, Git source, heading Git source, a toolbar of Update, Pull now, and Remove, and Status, Repository, and Manifest sections" />
</Frame>

Git Source opens as a right-side sheet from the stack editor toolbar. It is not a centered dialog. The chrome is the same sheet used elsewhere in Sencho: a crumb (`Stack › {name} › Git source`), the heading **Git source**, a one-line summary of the linked ref, a toolbar, body sections, and a freshness line in the footer.

The toolbar carries **Save** or **Update**, **Pull now** when a source is linked and not suspended, **Suspend** or **Resume** when that action is offered, and **Remove** when a source is linked. Nested pull/apply and detach confirmations stay as separate dialogs on top of the sheet.

When this Git source belongs to a Git-managed Blueprint, the sheet is read-only. **Save**, **Remove**, and **Pull now** are withheld here. Use **Detach Git** or **Retire to Direct** on that Blueprint to restore stack-owned editing. A live Direct source can **Adopt onto Blueprint**, which moves this source onto an Inline Blueprint with the same move semantics as Convert.

The body groups five sections:

- **Status.** Faults, a pending-update card with **Review**, retry or suspended state with **Retry** when that action is offered, qualifications the projection could not prove, and the last-applied commit with the source state.
- **Repository.** Repository URL, ref, the ordered compose-file picker, an optional project directory, optional sibling `.env` sync, authentication, and the apply behavior radio group.
- **Provider hooks.** One endpoint per supported Git host (GitHub, GitLab, Gitea, Forgejo, Bitbucket Cloud). Shown when the stack owns a direct Git source; hidden while the source is read-only on a Git-managed Blueprint.
- **Repository secrets.** Age identities for SOPS-encrypted env files and file-backed secrets, encrypted-source policy, and a readiness chip for whether this node can decrypt what the repository expects.
- **Manifest.** The managed-project summary when a source is linked.

Polling interval lives in **Settings → Stacks**, not on this sheet. There is no per-source interval control; a source with no override inherits the node default.

## Source state

A commit SHA tells you which files Sencho wrote. It does not tell you whether that commit was accepted, whether something newer is waiting, or whether an operation was interrupted halfway. The **source state** answers those, in one short phrase that means the same thing everywhere it appears: the Git Source sheet, the Drift tab, the stack list, and the stack rows on the dashboard.
...
| **Suspended** | Reconciliation is paused until you resume. Polling, webhooks, automatic apply, and Pull now stay off for this source. |
...
**Source state** tells you whether a commit was accepted. **Executable identity** tells you whether Sencho can prove which container images that generation would run, without rewriting your Compose files.

When a generation is accepted, Sencho resolves each service image reference against the target node platform. Registry tags become digest evidence. Multi-arch indexes record the platform-specific child digest for that node. Local `build:` services stay unverified across nodes because Sencho does not claim a portable exact digest for them.

After deploy, Sencho inspects what is actually running on the node and compares that observation to the frozen expected artifact set. If a registry tag moved after acceptance, Sencho records stale artifact evidence without changing the expected set. If the running digest disagrees with the expected fingerprint, runtime state reports artifact drift while source state can still read as accepted.

The Git Source sheet and the Drift tab show executable identity beside source and runtime so you can see configuration convergence separately from digest proof.

## Create a stack from a Git repository

Skip the "empty stack then link later" detour and point at a repo from the start. Click **Create Stack** in the sidebar, switch to the **From Git** tab, and fill in the same fields you would on the Git Source sheet.
...
**Review only** maps to review policy. **Auto-write files** and **Auto-deploy** map to automatic policy. Suspend is the operator stop; there is no fourth Manual radio on the sheet.

Automatic modes carry one built-in safety check: when the incoming commit withdraws or renames a service that persists data (named volumes or bind mounts), or when Sencho cannot prove it does not, the change is held for review instead of being accepted or applied, and the stack shows a **stateful change blocked** attention state. The hold clears when you accept the change explicitly, dismiss it, or a newer commit is staged. Nothing is written or redeployed while it is held, so withdrawing a stateful service is always a deliberate choice.

Git Sources apply to stacks you link from the editor. Blueprint deployments do not evaluate a Git source.

## Polling

Unattended fetches use a node-wide interval under **Settings → Stacks**. The control is **Poll Git sources**. The default is off (interval `0`). Turn it on to fetch linked sources on a schedule; sources still pull on demand, on webhook, and on retry when polling is off.

Linked sources with no override inherit this interval. There is no per-source interval knob on the Git Source sheet.

Review-only sources still fetch on the schedule and stage a plan for review. Automatic sources (Auto-write or Auto-deploy) fetch and then follow the configured apply behavior.

## Suspend, resume, and retry

**Suspend** pauses reconciliation for one source: polling, webhooks, and automatic apply stop until you resume. Save stays available; resume before pulling again. An optional reason is stored with the suspension.

**Resume** clears the pause and re-evaluates the source immediately rather than waiting for the next poll.

**Retry** is offered when the last attempt failed or a retry is already scheduled, and not while the source is suspended. Retry is stage-aware: a failed fetch refetches, and a failed apply or deploy continues from the evidence already on disk. Permanent failures (for example a missing ref) do not schedule an automatic retry; use **Retry** after you fix the cause.

When a deploy fails its [health gate](/features/health-gated-updates), Retry re-runs the deploy for the applied commit. With health gates enabled, the resumed deploy records a new gate verdict, so a later Retry falls back to refetching once that deploy succeeds. With health gates disabled, no new verdict is recorded and the earlier failed verdict stays the latest one, so each further Retry re-runs the deploy again instead of refetching. Every retry is still an explicit action you take; Sencho never loops on it unattended.

## Pulling and reviewing changes

Click **Pull now** on the Git Source sheet to fetch the latest commit on the configured branch, tag, or commit.
...
When a webhook fires in **Review only** mode, the stack gets a pending GitBranch icon next to its row in the sidebar and a pulsing dot on the **Git Source** button in the editor. Clicking either re-fetches the commit and opens the change plan; the sheet also shows a pending-update card with a **Review** button.
...
## Provider webhooks (native)

When your Git host can deliver push (and optionally pull request) events directly, register a **provider hook** on the Git Source sheet instead of calling Sencho from CI. Sencho exposes one ingest URL per stack and provider kind, verifies the host's signature or shared secret, and queues reconciliation through the same Git source pipeline as **Pull now** or a generic webhook.

Provider hooks apply only to **direct** stack-owned sources. A source that is read-only because it lives on a Git-managed Blueprint cannot register an endpoint; detach or retire the Blueprint binding first.

### Create an endpoint

1. Open the stack's **Git Source** sheet and scroll to **Provider hooks**.
2. Click **Add provider hook**, choose the Git host, and pick an **event scope**:
   - **Configured ref only** (default): react to push events on the branch or tag named in **Ref**.
   - **Configured ref and pull requests**: also accept pull request events from the same repository (useful when the ref tracks a long-lived branch and you want preview stacks from PR activity).
3. Copy the **ingest URL** and **secret** from the one-time reveal. Sencho never shows the full secret again; use **Rotate secret** to issue a new one (the previous secret stays valid for 24 hours so you can update the Git host without a gap).
4. In your Git host's repository settings, add a webhook pointing at the ingest URL and paste the secret using that host's expected field (GitHub: **Secret**; GitLab: **Secret token**; Gitea/Forgejo: **Secret**; Bitbucket Cloud: **Secret**).

The ingest URL always includes the target node id and endpoint id:

```
POST /api/gitops/hooks/{nodeId}/{endpointId}
```

Use the public Sencho base URL your Git host can reach. On a fleet hub, the node id in the URL is the node that owns the stack (usually **Local** for the instance you manage stacks on). Ingest is served on the control instance even when the stack runs on a remote node; the hub forwards the payload to the owning instance over the node mesh.

You can register at most one endpoint per provider per stack. A second create for the same provider returns **409**.

### What happens on delivery

Sencho authenticates every delivery with provider-specific headers, then applies policy before touching the repository:

| Check | Outcome when it fails |
| --- | --- |
| Unknown endpoint, disabled endpoint, or bad signature | **404 Not found** (same response for every rejection so callers cannot probe ids) |
| Malformed JSON or unsupported payload shape | **400 Bad request** |
| Body larger than 1 MiB | **413 Payload too large** |
| Repository in the event does not match the linked source URL | **202 Accepted**, delivery recorded as ignored (`source_mismatch`) |
| Push ref does not match the configured ref | **202 Accepted**, ignored (`ref_policy`) |
| Pull request event while scope is **Configured ref only** | **202 Accepted**, ignored (`ref_policy`) |
| Ping or connectivity test from the host | **202 Accepted**, ignored (`ping`) |
| Duplicate delivery id within the debounce window | **202 Accepted**, duplicate (same 10-second per-source debounce as generic webhooks) |

When a delivery passes policy, Sencho queues `handleWebhookPull` for the stack. The configured **Apply behavior** then governs the outcome exactly as it does for **Pull now**, polling, or the generic **Git source sync** webhook action:
...
Recent deliveries for each endpoint appear inline on the Git Source sheet (event type, ref, state, and outcome). Use **Test delivery** after saving the hook in your Git host to load the latest few results without waiting for a real push.

### Provider signing reference

| Provider | Header(s) Sencho verifies |
| --- | --- |
| **GitHub** | `X-Hub-Signature-256` (`sha256=` HMAC of the raw body) |
| **GitLab** | `X-Gitlab-Token` (shared secret) |
| **Gitea** | `X-Gitea-Signature` (hex HMAC-SHA256 of the raw body) |
| **Forgejo** | `X-Forgejo-Signature`, or `X-Gitea-Signature` when Forgejo omits its own header |
| **Bitbucket Cloud** | `X-Hub-Signature` (`sha256=` HMAC of the raw body) |

Delivery ids are taken from the host's idempotency headers when present (`X-GitHub-Delivery`, `X-Gitea-Delivery`, `X-Forgejo-Delivery`, `Idempotency-Key`, `X-Request-UUID`, and others) so retries dedupe cleanly.

Ingest is rate limited to 120 requests per minute per endpoint in production. Bursts above that return **429 Too many provider hook deliveries**.

## Trigger from CI with a webhook

When CI already orchestrates your pipeline, or your Git host cannot call Sencho directly, use Sencho's generic webhook system instead. Create a webhook under **Settings → Automation → Webhooks** targeting the stack with the **Git source sync** action. See [Webhooks](/features/webhooks) for signing and execution details.
...
The in-browser editor and the Git Source sheet both write to the same files, so you can always fall back to editing locally. The next pull will flag the divergence rather than silently clobbering your edits.
...
## Repository secrets (SOPS and age)

Some teams store sensitive env files or file-backed secrets in Git as [SOPS](https://github.com/getsops/sops)-encrypted documents. Sencho keeps ciphertext in the repository and on disk. Decryption happens only in a short-lived overlay during candidate validation, Git apply, Git deploy, or rollback. A manual Deploy or image Update on a SOPS-managed stack is refused so Compose never reads ciphertext from the live stack directory.

Open **Git Source** on a linked stack and scroll to **Repository secrets**.

- **Generate identity** or **Import identity** creates an age key pair on the node that owns this Git source. Sencho shows the public recipient (`age1…`) so you can add it to SOPS files in Git. Private key material never leaves the node and is never returned by the API.
- **Encrypted source policy** defaults to allowing plaintext secret inputs. Switch to **Require SOPS encryption** when every managed secret-bearing input in the repository must be SOPS-encrypted with age.
- **Readiness** reports whether this node holds the recipients required by the current candidate generation.

Sencho accepts age (X25519) recipients only, in YAML SOPS documents (`sops.age`) and in dotenv or INI stores that flatten age recipients (`sops_age__list_*` keys). Documents that use PGP, cloud KMS, or other SOPS backends are refused with an unsupported-backend message. Encrypted compose files are not supported.

Back up both the Sencho data directory encryption key (`encryption.key`) and any age identities you generate here. Losing either makes ciphertext in Git unrecoverable on that node.

## Troubleshooting

<AccordionGroup>
  <Accordion title="Missing or rotated SOPS age key">
    A Git apply, Git deploy, or rollback that needs SOPS decryption fails before Compose runs when this node does not hold a matching age identity. Open **Repository secrets**, confirm the readiness chip, and generate or import the key whose recipient appears in your SOPS files. After rotating a key, re-encrypt repository files with the new recipient before deleting the old identity. Rollback to a generation that still names a removed recipient stays blocked until you restore that identity or advance to a generation encrypted for a key you still hold. Manual Deploy and image Update stay blocked on SOPS-managed stacks; use Git apply instead.
  </Accordion>
  ...
  <Accordion title="File not found">
    Each compose path is relative to the repository root and must point at the file, not its parent directory. Every file in the list must exist on the tracked branch; if any is missing, the save or pull fails and names the path. If a file was moved, update its path on the sheet and save.
  </Accordion>
  ...
  <Accordion title="Pending commit has changed since this pull was fetched">
    You opened a change plan, then a webhook fired and replaced the pending commit before you clicked **Apply**. Close the dialog and reopen the sheet to load the latest pending commit; the **Review** button will fetch the newer one.
  </Accordion>
...
- **Git-managed Blueprint binding.** Convert or Adopt moves a live Direct source onto a Blueprint. That stack's Git Source sheet is then read-only until you Detach Git or Retire to Direct on the Blueprint. Git-managed Blueprints bind and show a blocked rollout state; they do not deploy from the stored snapshot. Fleet place for those Blueprints waits on registry preflight before rollout authorization. See [Blueprints](/features/blueprint-model).
