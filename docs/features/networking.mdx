...
The overview masthead summarizes the node at a glance and derives its posture from findings: action needed, review, contained, partial networking data, or runtime unavailable. The overview cards link to the relevant inventory or findings view. The operator attention card exposes the next safe action, while the activity band shows recent node-wide stack work.
...
Each finding includes a severity, evidence, and the actions that are appropriate for the current operator. Depending on permissions and the finding, this can open the affected stack, its Networking or Doctor tab, filter topology, inspect a network, create a missing network, copy a safe Compose snippet, or refresh the data.
