...
| Status | Meaning |
|--------|---------|
| **In sync** | Declared services match runtime: same running services, images, and published ports, or a declared one-shot finished cleanly (`Exited (0)` with no-restart intent: service `restart: "no"` / absent, or `deploy.restart_policy.condition: none`). |
