...
For a stack managed by a Git source or a Blueprint, a **gitops** section sits below the deploy signal. It carries the source state, which is the same phrase the Git Source panel shows, and one line per node saying what Sencho has established about the stack there: whether a generation has been applied, whether it is deployed, whether its health verdict is still outstanding, and whether an operation was interrupted without a confirmed outcome.
