...
**Opt-in.** Sencho writes a Compose override file (`<DATA_DIR>/mesh/overrides/<nodeId>/<stack>.override.yml`) that adds the `sencho_mesh` network attachment to every service in the stack and injects `extra_hosts` lines for every cross-node alias the fleet currently exposes. Services keep their existing networks: a service that relies on the stack's implicit `default` network stays on it, so links between services in the stack and routes from your reverse proxy keep working. Services that use `network_mode` (`host`, `none`, `service:<name>`, `container:<name>`) are left out of the override, because Compose does not allow extra networks or hosts entries on them. The stack redeploys with that override applied. Containers come back up with the alias names resolvable in their `/etc/hosts`.
...
**Inbound exposure.** Sencho listens for mesh traffic only on its own `sencho_mesh` address, so containers on other networks Sencho is attached to (for example a reverse proxy's network) cannot reach meshed services through it. Mesh adds no new inbound port to central. A Pilot Agent node's outbound tunnel carries mesh traffic in both directions. A Distributed API Proxy node only needs the inbound port Sencho already listens on for fleet operations (so it can be dialed by central). Aliases themselves are not internet-reachable; they only resolve inside the `sencho_mesh` Docker bridge on each participating node.
...
- **`network_mode` services cannot join.** A service that uses `network_mode` (`host`, `none`, or another container's namespace such as a VPN sidecar with `service:gluetun`) cannot attach to `sencho_mesh`. The rest of the stack still meshes; Sencho leaves those services untouched. Alias discovery reads published ports, so such a service can still show an alias; whether it routes depends on how the rest of the stack exposes that port.
...
<AccordionGroup>
  ...
  <Accordion title="A service using network_mode is left untouched">
    Services that declare `network_mode` (`host`, `none`, `service:<name>`, `container:<name>`) are left out of the mesh override, because Compose rejects extra networks and hosts entries on them. They keep running on the namespace they already use. For a VPN sidecar pattern, the ports come from the sidecar, so reach the stack through the sidecar's service. Otherwise switch the service to bridge networking and redeploy.
  </Accordion>
