...
1. **The `sencho_mesh` Docker bridge.** Each Sencho instance creates a Docker bridge network on first boot (default `172.30.0.0/24`) and pins itself to the first usable host address on it. This is the lane every meshed container uses to talk to the local Sencho.
...
**Peer reconnect.** Central recomputes the global alias set on every alias-refresh tick (60 s). It rewrites a node's override files when that node's tunnel comes back, when a stack is opted in or out anywhere in the fleet, or when you ask for a regeneration. Running containers are not recreated by any of this: `/etc/hosts` entries are read when a container is created, so they pick up a changed alias set on the stack's next redeploy, or on the next opt-in or opt-out anywhere in the fleet (which redeploys every meshed stack).
...
**Encryption in transit.** The WebSocket between Sencho instances is whatever TLS posture you already configured for fleet management. If you front Sencho with a TLS-terminating reverse proxy, mesh inherits that. If you use a private CA: for Pilot Agent nodes set [`SENCHO_PILOT_CA_FILE`](/features/pilot-agent#environment-variables) on the agent; for Distributed API Proxy nodes set `SENCHO_MESH_PROXY_CA_FILE` on central (see [Reverse proxies, tunnels, and VPNs](#reverse-proxies-tunnels-and-vpns)).
...
- **Idle connections close after 10 minutes.** A mesh TCP connection with no traffic in either direction for 10 minutes is closed. Database pools and long-lived subscriptions (Redis pub/sub, MQTT) should use keepalives or reconnect on their own.
- **Connections do not survive a transport restart.** When a tunnel reconnects or central restarts, open mesh connections are reset and applications must reconnect. Traffic between two remote nodes is relayed through central, so central being down stops it.
...
## Reverse proxies, tunnels, and VPNs

Mesh does not care how the nodes reach each other, as long as the path carries a WebSocket.

- **Pilot Agent nodes** open an outbound WebSocket to central, so the agent side needs no inbound port, tunnel, or proxy route. The path to central must allow WebSocket upgrades on `/api/pilot/tunnel`.
- **Distributed API Proxy nodes** are dialed by central at `/api/mesh/proxy-tunnel` on the node's API URL. That route must reach the remote Sencho with the `Upgrade` and `Connection` headers intact.

**Reverse proxies (nginx, Traefik, Caddy, NPM).** Enable WebSocket proxying for the Sencho host. Traefik and Caddy do this automatically; for nginx, set `proxy_http_version 1.1` and forward the `Upgrade` and `Connection` headers. A ping goes out every 30 seconds, so default proxy idle timeouts (60 s and up) are enough.

**Cloudflare Tunnel and proxied DNS.** WebSockets work through Cloudflare without extra settings, and the 30-second pings keep the connection inside Cloudflare's 100-second idle limit. If a Cloudflare Tunnel or edge connection restarts, the dead connection is detected within about a minute and central reconnects on its own. Mesh TCP connections that were open at that moment are reset, so applications reconnect.

**Access gateways (Cloudflare Access, Authelia, Authentik).** A gateway that demands its own login blocks central's dial; the node card shows `blocked by an access proxy`. Either exempt `/api/mesh/proxy-tunnel` (and the rest of `/api/`, which fleet management also uses) from the gateway, or give central a service credential. For Cloudflare Access service tokens:

```yaml
services:
  sencho:
    environment:
      - 'SENCHO_MESH_PROXY_HEADERS={"CF-Access-Client-Id":"<id>","CF-Access-Client-Secret":"<secret>"}'
```

The value is a JSON object of header names to values, applied to every proxy-mode node. For different credentials per node, key the object by node name, with `"*"` as the fallback: `{"*": {...}, "edge": {...}}`. Headers that Sencho manages (`Authorization`, `Host`, `Upgrade`, `Connection`, `Sec-WebSocket-*`) cannot be overridden, and a node whose API URL is not HTTPS is skipped rather than having the credential sent in cleartext. The headers apply to the mesh connection only, not to other fleet management requests.

**Private certificate authorities.** When a remote serves a certificate from an internal CA, mount the CA bundle into central and set `SENCHO_MESH_PROXY_CA_FILE` to its path inside the container. The bundle is trusted in addition to the default roots (and `NODE_EXTRA_CA_CERTS`). Certificate verification is never disabled.

**VPNs (Tailscale, WireGuard, ZeroTier).** Point the node's API URL at the VPN address or MagicDNS name. Private and CGNAT ranges (`10/8`, `172.16/12`, `192.168/16`, `100.64/10`) are allowed. Loopback, link-local, and reserved ranges (including the `198.18/15` range some proxy tools use for fake IPs) are refused, so do not use an SSH tunnel to `localhost` as a node URL. If a VPN reconnects or changes address, the old connection is detected dead within about a minute and central redials.

**Mesh subnet and VPN routes.** Docker does not check a mesh subnet against the host's routes. If the host routes a VPN, Kubernetes (k3s uses `10.42.0.0/16` and `10.43.0.0/16`), or cloud VPC range that overlaps the chosen `/24`, traffic from the host to that range goes to the mesh bridge instead. On AWS, `172.31.0.0/24` falls inside the default VPC. Check `ip route` on the host and set `SENCHO_MESH_SUBNET` to a range nothing else uses.
...
<AccordionGroup>
  ...
  <Accordion title="A node card shows `unreachable`">
    ...
    - `node token is not full-admin`: the remote accepted the token but its scope cannot carry mesh traffic. Generate a full-admin Node Token on the remote.
    - `license tier does not include mesh`: the remote refused because mesh is not included in the tier.
    - `TLS handshake failed`: the TLS handshake did not complete. If the remote serves a certificate from a private CA, mount your CA bundle into central and set `SENCHO_MESH_PROXY_CA_FILE`; otherwise check that the node URL scheme matches what the server actually speaks, and that the certificate is valid and unexpired.
    - `blocked by an access proxy in front of the remote`: something in front of the remote Sencho (Cloudflare Access, Authelia, Authentik, a WAF) refused or redirected the request before it reached Sencho. Add a bypass for `/api/mesh/proxy-tunnel`, or send the gateway's credential with `SENCHO_MESH_PROXY_HEADERS`. See [Reverse proxies, tunnels, and VPNs](#reverse-proxies-tunnels-and-vpns).
    - `WebSocket upgrade refused`: the path answered with plain HTTP instead of a WebSocket upgrade. The reverse proxy is not forwarding the `Upgrade` and `Connection` headers, or the URL points at the wrong port.
    - `reverse proxy or tunnel in front of the remote cannot reach Sencho`: the proxy or tunnel answered with a 502, 503, 504, or a Cloudflare 52x error. Check that the Sencho container is running and that the proxy's upstream address is correct.
    - `remote hostname does not resolve`, `connection refused by remote host`, `remote did not answer in time`: DNS, port, or firewall problems between central and the remote.
    - `remote address is loopback or reserved and cannot be dialed`: the node URL resolves to a loopback, link-local, or reserved address, which Sencho never dials. Use an address central can route to (LAN, VPN, or public).
    - `mesh dial settings are invalid`: `SENCHO_MESH_PROXY_HEADERS` or `SENCHO_MESH_PROXY_CA_FILE` could not be parsed, or configured headers were skipped because the node's API URL is not HTTPS. The mesh activity log names the problem.
    - `the remote Sencho refused the mesh connection`: the remote rejected the connection without giving a reason central could classify. Check the node token's scope and the remote's mesh configuration.
