...
Each opt-in and opt-out restarts the affected stack so its `/etc/hosts` entries and `sencho_mesh` network attachment refresh, and also restarts every other meshed stack in the fleet so they learn the new hostnames. The confirmation says exactly what will restart before you commit, for example: `api on vps restarts now. 4 other meshed stacks on 3 nodes also restart to refresh their hostnames, so expect brief connection drops.` If any of those stacks is not currently running, the confirmation says so too, because the fleet-wide deploy brings it up. Turning mesh off on a node asks for the same confirmation, because it restarts that node's meshed stacks and the rest of the fleet.

Every hostname on a node card and in the route detail sheet has a copy button that copies `host:port`, ready to paste into a connection string.
...
- **Success.** `<alias> is reachable (<rtt>ms)`.
- **Failure.** A sentence that says what went wrong, with a button to the place to fix it (for example **Open diagnostics** when the connection to the node is down). The route detail sheet shows the same sentence. Internally the probe reports the stage it failed at:
...
**Triage a "my app can't connect."** A developer reports their app stack can't reach `db.api.opsix.sencho`. Open Fleet → Routing and click the Test button on the alias row. A green toast confirms the mesh path is healthy and the issue is inside the application. A red toast saying the connection to `opsix` is down offers **Open diagnostics** for that node; use it with the Mesh activity log to chase the tunnel state.
