...
**Node offline.** When Sencho cannot determine a node's current state, it keeps the aliases and ports that node's meshed stacks last published. That covers an offline node and a node that answers with an error, such as after its API token rotates. Other nodes' overrides still list those hostnames (so redeploying a stack elsewhere does not drop them), and no other stack can claim those ports while the node is away. Connections to the node fail until it responds again. Stopped stacks release their aliases but keep their ports reserved, so a stopped stack never frees a port for a different stack to take. The reservation is released when you opt the stack out, so the recovery is to wait for the node or remove it from the fleet.
