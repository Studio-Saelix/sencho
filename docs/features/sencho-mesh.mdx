...
- **`network_mode` services cannot join.** A service that uses `network_mode` (`host`, `none`, or another container's namespace such as a VPN sidecar with `service:gluetun`) cannot attach to `sencho_mesh` and therefore cannot publish an alias. The rest of the stack still meshes; Sencho leaves those services untouched.
...
<AccordionGroup>
  ...
  <Accordion title="A service using network_mode has no alias">
    Services that declare `network_mode` (`host`, `none`, `service:<name>`, `container:<name>`) are left out of the mesh override and publish no alias. For a VPN sidecar pattern, the ports are published by the sidecar service, so they are reachable through the sidecar's alias. Otherwise switch the service to bridge networking and redeploy.
  </Accordion>
