...
**Node offline.** When a node cannot be reached, Sencho keeps the aliases and ports its meshed stacks last published. Other nodes' overrides still list those hostnames (so redeploying a stack elsewhere does not drop them), and no other stack can claim those ports while the node is away. Connections to the offline node fail until it reconnects. Stopped stacks also keep their ports reserved.
