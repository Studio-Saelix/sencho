...
**Peer reconnect.** On every alias-refresh tick (60 s), and immediately on a tunnel-up event, central recomputes the global alias set and rewrites the override files on the reconnected node. Running containers are not recreated by this: `/etc/hosts` entries are read when a container is created, so they pick up a changed alias set on the stack's next redeploy, or on the next opt-in or opt-out anywhere in the fleet (which redeploys every meshed stack).
...
**Reverse proxies (nginx, Traefik, Caddy, NPM).** Enable WebSocket proxying for the Sencho host. Traefik and Caddy do this automatically; for nginx, set `proxy_http_version 1.1` and forward the `Upgrade` and `Connection` headers. Both ends send a ping every 30 seconds, so default proxy idle timeouts (60 s and up) are enough.
...
The value is a JSON object of header names to values, applied to every proxy-mode node. For different credentials per node, key the object by node name, with `"*"` as the fallback: `{"*": {...}, "edge": {...}}`. Headers that Sencho manages (`Authorization`, `Host`, `Upgrade`, `Connection`, `Sec-WebSocket-*`) cannot be overridden. The headers apply to the mesh connection only, not to other fleet management requests.
...
<AccordionGroup>
  ...
  <Accordion title="A node card shows `unreachable`">
    ...
    - `TLS certificate not trusted`: the remote serves a certificate central does not trust. Use a publicly trusted certificate, or mount your CA bundle into central and set `SENCHO_MESH_PROXY_CA_FILE`.
    ...
    - `mesh dial settings are invalid`: `SENCHO_MESH_PROXY_HEADERS` or `SENCHO_MESH_PROXY_CA_FILE` could not be parsed. The mesh activity log names the problem.
