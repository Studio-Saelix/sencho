...
2. Type a name in the **Stack label** field. The picker suggests stack labels from across the fleet, each with its stack and node counts, so the scope is unmistakable. Suggestions are read live from each reachable node, so a stack label that exists only on a remote appears too; a label on an offline node cannot be read, so type the name by hand to target it. You can also type a name by hand at any time.
3. Click **Stop fleet**.
...
<AccordionGroup>
  ...
  <Accordion title="The autocomplete didn't suggest a stack label I know exists">
    The picker reads stack labels live from each reachable node, so a name that exists only on a remote still appears, but a label on an offline node cannot be read. Node labels never appear here, because these actions target stack labels only. You can always type a name by hand; the fleet-stop request still runs authoritatively against every reachable node.
  </Accordion>
