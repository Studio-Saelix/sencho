...
- **Reduced effects** flattens card bevels, the accent glow, and chart gradients, and uses a simpler rendering path: glass blur is off and chrome fills are solid. Fresh installs get this through the default **Calm** visual style. It is intended for constrained browser clients (integrated graphics, browsers on single-board computers, and similar devices) where full material effects can be costly. It does not claim a measured GPU utilization change on every device.
- When full effects are on, Appearance shows a short notice about constrained rendering devices and points you at Reduced effects or Calm.
...
If the active Sencho tab feels heavy on a constrained GPU, see [High GPU usage with full material effects](/operations/troubleshooting#high-gpu-usage-with-full-material-effects).
