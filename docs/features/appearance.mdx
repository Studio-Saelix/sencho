...
- **Stack detail layout** applies the same choice to the Anatomy panel on the right side of Stack details. **Fixed** uses the standard even split. **Resizable** lets you drag or keyboard-adjust the divider to set a preferred Anatomy width between 320 and 960 pixels. **Reset Anatomy panel layout** restores the even split and default preferred width.
...
Fixed mode preserves the standard even split. If the window becomes too narrow for your preferred width, Sencho temporarily clamps the Anatomy panel so the left pane stays usable. The saved width returns when space is available again. The divider is not applied to the compose editor or phone layout.
