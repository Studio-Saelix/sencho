...
- **Reduced effects** flattens card bevels, the accent glow, and chart gradients, and uses a simpler rendering path: glass blur is off and chrome fills are solid. Fresh installs get this through the default **Calm** visual style. It is intended for constrained browser clients (integrated graphics, browsers on single-board computers, and similar devices) where full material effects can be costly. The impact varies by device, browser, and graphics configuration.
