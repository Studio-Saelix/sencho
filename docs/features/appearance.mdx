---
title: Appearance
description: Personalize Sencho's look and desktop pane layout. Choose a visual style, tune readability, set the sidebar and Anatomy panel widths, and save every choice to your account.
---

**Settings → Appearance** is where you make Sencho look and fit the way you want. Choose between the Calm and Signature visual styles, set how the security charts are colored, dial in readability, pick a theme and accent, choose the interface and data fonts, scale the text, and set the desktop pane widths. A live preview reflects visual changes as you make them.
...
The **Display** group holds layout and log-chip preferences for your account:
...
- **Sidebar layout** chooses how the desktop stacks sidebar is sized: **Fixed** (the default) keeps it at its standard width, and **Resizable** makes the divider between the sidebar and the workspace draggable. Drag the divider, or focus it and use the arrow keys, to set any width between 248 and 440 pixels; the width is saved when you release the divider, or with each arrow-key press. A window too narrow for the chosen width temporarily clamps it, and the full width returns when the window is wide again. **Reset sidebar layout** returns the sidebar to the Fixed default and the default width.
- **Stack detail layout** applies the same choice to the Anatomy panel on the right side of Stack details. **Fixed** uses the standard even split. **Resizable** lets you drag or keyboard-adjust Anatomy from 320 pixels up to the 4096-pixel preference ceiling, with a viewport clamp that keeps the other pane at least 320 pixels wide. **Reset Anatomy panel layout** restores the even split and default preferred width.

## Sidebar

The **Sidebar layout** section changes the stacks list on every desktop page, so the workspace gains the room that dense stack names, wide tables, and terminal output need (or gives it back when you want the sidebar compact).

<Frame>
  <img src="/images/settings/appearance-sidebar-layout.png" alt="The Settings → Appearance page showing the Sidebar layout section with the Fixed and Resizable segmented control set to Resizable, a width slider reading 256px, and the Reset sidebar layout button below, with the stacks sidebar and its resize divider visible at the left edge of the page behind the settings panel." />
</Frame>

<Note>
  The sidebar layout is saved to your account with the rest of your appearance preferences. Phone screens keep their own layout and ignore this setting.
</Note>

## Anatomy panel

The **Stack detail layout** section controls the right-hand Anatomy panel while a stack is open. Choose **Resizable**, then drag the divider left to give Anatomy more room or right to give status and logs more room. Focus the divider and use the arrow keys for precise adjustments. The width is saved on release and follows your account across devices.

<Frame>
  <img src="/images/settings/appearance-anatomy-layout.png" alt="Stack details with the Anatomy panel on the right and its focused vertical resize divider between Anatomy and the stack status and logs pane." />
</Frame>

Fixed mode preserves the standard even split. In Resizable mode, either pane can take the available space until the other reaches 320 pixels, subject to the 4096-pixel Anatomy preference ceiling. If the window becomes too narrow for your preferred width, Sencho temporarily clamps the Anatomy panel without changing the saved preference. The saved width returns when space is available again. The divider is not applied to the compose editor or phone layout.

<Note>
  Sidebar and Anatomy layouts are independent. Resetting one does not change the other or any visual preference.
</Note>
...
- **Quick links** (Compact launcher): labeled pins after the launcher, with a trailing **+** that opens reachable unpinned destinations. Right-click a pin and choose **Remove from quick links**, or manage the full list under Appearance. Up to eight pins; recommended defaults start you with a reachable set of up to six.
...
<AccordionGroup>
  <Accordion title="My theme didn't carry over to another device or browser">
    Appearance is saved to your Sencho account, so it follows you to any browser or device where you sign in. If a fresh browser shows the defaults, give it a moment after sign-in: your saved look arrives as the page loads. If it still shows the defaults, check that you signed in with the same account.
  </Accordion>
  <Accordion title="My appearance reset to the defaults">
    If your account's saved preferences cannot be read, Sencho falls back to the defaults: **Dim** mode, **Cyan** accent, **Geist** and **Geist Mono** fonts, **1.00×** text size, and **Comfortable** density. Sign out and back in to re-fetch your saved choices; anything you pick while offline is kept locally and syncs the next time the app syncs with the server, such as after you sign back in or reload. Clearing site data does not remove your saved preferences, because the browser copy is only a cache for first paint.
  </Accordion>
  <Accordion title="Will changing my accent or theme affect other operators?">
    No. Appearance is personal to your account and is never shared. Other operators keep whatever look they set for their own accounts; your changes affect only the pages you see. There is no shared or organization-wide appearance setting; each person chooses their own.
  </Accordion>
  <Accordion title="The quick switcher doesn't have the contrast, border, and glow sliders">
    The top-bar palette button is a fast path for the most-used controls: mode, accent, visual style, readability, and text size. Contrast, border brightness, and ambient glow live here in **Settings → Appearance**, where the live preview reflects each change as you drag. Both paths write to the same saved preferences, so a change in one shows up immediately in the other.
  </Accordion>
</AccordionGroup>
