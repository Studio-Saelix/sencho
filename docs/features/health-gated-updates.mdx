...
A one-shot or init service that finishes with exit code `0` and has restart policy `no` (or no restart policy) counts as a successful completion, not a gate failure. Residual Docker health state on that completed container (`starting` or `unhealthy` from an inherited healthcheck) does not fail or leave the gate unknown. A clean exit under `unless-stopped`, `always`, or `on-failure` still fails the gate. An exit whose code cannot be read also fails the gate.
