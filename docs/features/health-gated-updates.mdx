...
- **Recovery generation**: whether a current generation exists for exact restore. Atomic deploy, full-stack update, and Git apply capture one.
...
<Frame>
  <img src="/images/health-gated-updates/dossier-rollback-readiness.png" alt="Rollback readiness section in the Stack Dossier showing the overall state chip and rows for recovery generation, managed inputs, compose and env coverage, previous image identity, last successful deploy, healthchecks, rollback eligibility, and application data marked not covered" />
</Frame>
