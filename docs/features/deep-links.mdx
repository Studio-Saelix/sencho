...
Fleet tab URLs are stable slugs that do not always match the tab's on-screen label. For example the **Readiness** tab uses the `readiness` segment and the **Map** tab uses the `dependencies` segment. The `configuration` segment also resolves to **Readiness**.
