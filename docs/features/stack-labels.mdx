...
<Note>
  Stack Labels require a Sencho **Skipper** or **Admiral** license. Community Edition hides the trailing dots on stack rows, hides the **Settings · Advanced · Labels** section, and hides the **Fleet · Fleet Actions** tab. Existing label rows in the database are preserved if a node downgrades, so re-licensing brings the grouping back without manual rework.
</Note>
...
Two cards in the **Fleet · Fleet Actions** tab use labels to drive cross-node operations. Both endpoints are gated on a paid tier and an admin role; Community installs see a single empty-state card on the tab explaining the gate, and a non-admin operator on a paid tier sees the cards rendered but receives a 403 from the backend on submit.
...
- **Tier visibility**: Community hides trailing dots on stack rows and the entire Settings panel; the API endpoints that back labels stay live but unreachable from the UI. Re-licensing to Skipper or Admiral exposes everything again with no migration step.
...
<AccordionGroup>
  ...
  <Accordion title="The trailing colored dots are missing on stack rows">
    Trailing dots are gated on the paid tier. Community installs hide them on every row even when label rows exist on the node. The group headers themselves stay visible because they convey the same information; the row dots are an at-a-glance affordance for paid users. Upgrade to Skipper or Admiral to bring them back.
  </Accordion>
