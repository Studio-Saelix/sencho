...
4. Complete the Lemon Squeezy checkout. A valid card is required for verification; you are not charged until the trial ends.
5. Lemon Squeezy emails the license key within a few minutes.
6. Activate the key in the Sencho dashboard as described in [Activating your license](#activating-your-license).

When your trial ends, Lemon Squeezy automatically charges the card you provided and your plan continues as a paid Admiral subscription. To avoid being charged, cancel from the **Manage subscription** button in **Settings → License** (or from the Lemon Squeezy receipt email) any time before day 14.
...
Sencho validates the key against Lemon Squeezy and unlocks your tier. If activation fails, the toast surfaces the Lemon Squeezy error verbatim; common causes are an invalid key, a typo, or an activation limit reached on a previous instance.
...
- **Customer**: the customer name from your Lemon Squeezy account.
- **Product**: the Lemon Squeezy product variant (e.g. `Sencho Admiral`).
...
When the license is an active subscription (not lifetime), the Plan section action row exposes **Manage subscription** alongside **Deactivate**, with the hint "Lemon Squeezy manages billing" beside them. **Manage subscription** opens the Lemon Squeezy billing portal in a new tab, where you can update your payment method, view invoices, cancel, or switch plans.
