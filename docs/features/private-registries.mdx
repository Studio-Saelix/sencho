...
<AccordionGroup>
  ...
  <Accordion title="A remote operation fails with 'Registry credential delivery requires a confidential transport'">
    The hub proved the operation references a private image, and the connection to that node does not guarantee confidentiality. Use an HTTPS URL for the node's API address, or reach the node through a Pilot tunnel terminated with TLS or a trusted reverse proxy. To keep operating over a plain HTTP connection, configure the registry on that node's own Sencho instance instead; credentials stored on the node itself never need to travel.
  </Accordion>
