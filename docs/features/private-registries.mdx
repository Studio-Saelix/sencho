...
<AccordionGroup>
  ...
  <Accordion title="A remote operation fails with 'Registry credential delivery requires a confidential transport'">
    The hub proved the operation references a private image, and the connection to that node does not guarantee confidentiality. Use an HTTPS URL for the node's API address, terminate TLS on the hub with `SENCHO_TLS_CERT_FILE` and `SENCHO_TLS_KEY_FILE`, or reach the node through a Pilot tunnel in front of a trusted reverse proxy. See [Native TLS on the control instance](/getting-started/configuration#native-tls-on-the-control-instance). To keep operating over a plain HTTP connection, configure the registry on that node's own Sencho instance instead; credentials stored on the node itself never need to travel.
  </Accordion>
  ...
  <Accordion title="A remote operation fails because the transport is not confidential after a challenged reference">
    The operation requires a private image, the hub has credentials for the registry host, but the connection to the target does not guarantee confidentiality. Use an HTTPS URL for the node's API address, terminate TLS on the hub with `SENCHO_TLS_CERT_FILE` and `SENCHO_TLS_KEY_FILE`, or reach the node through a Pilot tunnel in front of a trusted reverse proxy. See [Native TLS on the control instance](/getting-started/configuration#native-tls-on-the-control-instance). To keep operating over a plain HTTP connection, configure the registry on that node's own Sencho instance instead; credentials stored on the node itself never need to travel.
  </Accordion>
