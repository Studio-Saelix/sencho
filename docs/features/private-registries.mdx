...
Open **Settings → System → Registries**. The section appears on the Sencho instance you are signed into directly, for admin operators on an Admiral tier license. It is hidden when you are viewing another node through the node switcher.
...
Registry credentials are stored on the Sencho instance where you enter them, and each instance resolves its own credentials at deploy time. Credentials are never copied between instances: a remote node authenticates its pulls with the registries configured on that node, not with the ones on the instance you happen to be signed into.
