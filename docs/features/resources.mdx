...
<Note>
  Sencho protects its own image, network, named volumes, and container from accidental deletion. They appear in the Images, Networks, and Volumes tabs with a **Sencho** pill alongside the managed status, and the delete button is disabled with a "Protected · running Sencho instance" tooltip. The Unmanaged tab never lists the Sencho container itself.
</Note>
