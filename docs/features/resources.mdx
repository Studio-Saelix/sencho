---
title: Resources Hub
description: Browse, filter, search, and clean up Docker images, volumes, networks, and unmanaged containers from a single view.
---
...
- **Scan** (shield) opens a menu with `Scan (vulnerabilities)` and `Full scan (vulnerabilities + secrets)`.
...
Lists all Docker networks with ID, name, driver, scope (`local`, `global`, `swarm`), and managed status. Click **Name**, **Driver**, or **Scope** to sort. System networks (`bridge`, `host`, `none`) are shown but cannot be deleted.
...
The footer shows a **DRIVER** chip that mirrors the selected driver, and a **Create network** button that stays disabled until you enter a name. Subnet and gateway, if provided, are validated on submit.
...
- **Overview** lists the ID (with a copy button), driver, scope, creation date, and the internal/attachable flags.
...
<Frame>
  <img src="/images/networks/network-topology.png" alt="Network topology graph with arr-net at top and the connected containers fanned out below it" />
</Frame>
...
- A **mini-map** gives an overview of the full graph, with zoom and fit-view controls alongside it.

On a remote node running a Sencho version too old to advertise the topology capability, this view shows a lock card explaining the version mismatch instead of the graph; upgrade that node to restore it. The List view and every other Resources action keep working in the meantime.
...
<Frame>
  <img src="/images/networks/network-topology-toggle.png" alt="Topology with Show system networks toggled on; arr-net plus the bridge system network both visible" />
</Frame>
...
<AccordionGroup>
  ...
  <Accordion title="The Networks tab's Topology view shows a lock card instead of the graph">
    The active node does not advertise the network-topology capability, which means it's running a Sencho version too old to support it. Upgrade that node; the List view and every other Resources action keep working in the meantime.
  </Accordion>
</AccordionGroup>
