...
<Tip>
  Community-tier operators can still integrate Google, GitHub, Okta, or any other identity provider by configuring the **Custom OIDC** option against the provider's discovery endpoint. The Skipper presets save a few clicks by shipping with the issuer URL pre-filled and a branded login button; you still create the OAuth app in the provider's console and paste the Client ID and Secret either way.
</Tip>
