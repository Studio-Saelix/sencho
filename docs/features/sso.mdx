...
Sencho lets your team sign in with the identity provider you already use instead of maintaining a second set of credentials. By default, SSO works alongside local password authentication. On Sencho Admiral you can switch to **SSO only** so interactive local password login is disabled and only configured LDAP or OIDC providers are accepted.

SSO provider configuration is available on every Sencho tier. Custom OIDC and the preset providers for Google, GitHub, and Okta work on every tier; LDAP and Active Directory require Sencho Admiral. SSO-only authentication mode requires Sencho Admiral.
...
- **SSO only** (Sencho Admiral): local password login is disabled. The login page shows only LDAP and/or OIDC providers. Direct calls to the password login endpoint are rejected.
