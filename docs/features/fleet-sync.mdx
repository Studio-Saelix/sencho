...
- **The built-in status signal is a rollup, not a push-by-push history.** [Fleet View](/features/fleet-view)'s **Readiness** tab carries a **Policy sync** cell per proxy-mode remote: `in sync`, `sync failing` when pushes are failing, `paused` when the remote's anchor no longer matches the control, or `never synced`. It tells you the current state only, never the history of individual pushes. For that detail, use the receiving replica's audit log (every applied push records `POST /api/fleet/sync/<resource>` with username `system` and the source fingerprint in the summary) or the control's server logs.
...
<AccordionGroup>
  ...
  <Accordion title="I cannot see whether sync succeeded">
    Open the [Fleet View](/features/fleet-view) **Readiness** tab and read the **Policy sync** column: `in sync`, `sync failing` when pushes are failing, `paused` when the remote's anchor no longer matches the control, or `never synced`. A failing or paused remote also lists a finding that carries the last push error; a paused remote's finding reads `Policy sync is paused on this node`. For push-by-push detail beyond that rollup (see [Limitations](#limitations)), two surfaces give you the information directly: the replica's **Audit** view records every applied push as a `POST /api/fleet/sync/<resource>` entry with username `system` and the source fingerprint in the summary, and the control's server logs record the push outcome per remote. The administrative API endpoint `GET /api/fleet/sync-status` returns the per-resource per-node status table for tooling that needs to consume it directly.
  </Accordion>
