...
In the **By label** layout, a **View labels defined by:** row of checkboxes lets you narrow to specific provenance (**Image**, **Runtime**, **System**, and **Unknown** when present); only the provenance types actually found are offered. Automation tools such as Watchtower, Diun, and Traefik read these labels, so the audit makes it easy to confirm which containers are opted in or out.
...
Because this tab renders the compose model, it can identify Compose-declared labels accurately. A toolbar at the top lets you narrow the view: a **search box** matches label keys and values as well as service and container names, a **View labels defined by:** checkbox row filters by provenance (**Compose File**, **Image**, **Runtime**, **System**), and a **Services** checkbox row shows or hides individual service cards. Matching a service or container name reveals that parent's labels even when the text does not match a specific key or value. Reconciliation counts always reflect only the labels currently visible.
