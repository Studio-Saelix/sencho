...
If you run the agent inside an LXC (including Docker-in-LXC on Proxmox), host CPU and memory gauges use the OS view inside the container. Keep that view truthful for the LXC (typically via **lxcfs** on the hypervisor host). No change to the enrollment Compose file is required. See [Nested LXC and Docker-in-LXC host metrics](/getting-started/configuration#nested-lxc-and-docker-in-lxc-host-metrics).
