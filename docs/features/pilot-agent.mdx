...
The enrollment Compose file includes `cgroup: host` so host resource gauges on nested LXC hosts can read the node's cgroup limits. The running service definition must keep that key; updating the agent image alone cannot add it to an older Compose file.
