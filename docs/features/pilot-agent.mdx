...
| **Network at the control instance** | The path from the remote to the control instance must allow WebSocket upgrades end-to-end. If you terminate TLS at a reverse proxy in front of Sencho, that proxy needs `Upgrade: websocket` passthrough. If Sencho terminates TLS itself, see [Native TLS on the control instance](/getting-started/configuration#native-tls-on-the-control-instance). |
...
The agent dials `wss://` when `SENCHO_PRIMARY_URL` is `https://`. Certificate validation is on by default against the system trust store. There is no flag to disable verification; that would defeat the credential model. For internal CAs, mount your bundle into the agent container and set `SENCHO_PILOT_CA_FILE` to the in-container path. When the control instance uses native TLS with `SENCHO_TLS_CA_FILE` set, enrollment already includes that bind-mount. See [Self-signed control-instance TLS certificates](#self-signed-control-instance-tls-certificates) below for a worked example.
...
If your control instance terminates TLS with an internal CA that the agent's system trust store does not recognise, mount the CA bundle into the agent container and point `SENCHO_PILOT_CA_FILE` at it. Native TLS on the hub with `SENCHO_TLS_CA_FILE` set does this for you in the generated compose file: save the PEM from the enrollment dialog as `sencho-hub-ca.pem` next to `compose.yaml`.

If you are writing the agent compose file by hand:
...
| Variable | Required? | Default | Notes |
|---|---|---|---|
| `SENCHO_PUBLIC_URL` | Optional | request Host | Locks in the URL the enrollment dialog bakes into `SENCHO_PRIMARY_URL`. See [The control instance's public URL](#the-control-instances-public-url) above. Must be `http(s)://`, no loopback. Use `https://` when native TLS is enabled. |
| `SENCHO_TLS_CERT_FILE` / `SENCHO_TLS_KEY_FILE` | Optional | unset | Enable the hub's own HTTPS listener. Ignored if set on a Pilot agent; the loopback listener stays HTTP. See [Native TLS on the control instance](/getting-started/configuration#native-tls-on-the-control-instance). |
| `SENCHO_TLS_CA_FILE` | Optional | unset | CA bundle copied into Pilot enrollment so agents trust a private issuer. |
