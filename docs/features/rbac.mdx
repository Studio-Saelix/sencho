...
**Node scopes are node-wide.** The resource is the node itself. There is no separate node qualifier on a node assignment row; granting Node Admin on `staging-server` covers that whole node.

The box appears below the user form whenever you are editing a user on Admiral.

<Frame>
  <img src="/images/rbac/scoped-permissions.png" alt="Edit User form for the viewer account with a Scoped Permissions box below. The box contains an existing assignment row (a Deployer badge with the text on Stack: bazarr @ prod and a destructive trash icon on the right) and an add-scope row underneath with Role, Resource Type, Node, and Stack controls (Role set to Deployer, Resource Type set to Stack, Node selected, Stack showing Select..., and a disabled Add button)." />
</Frame>
