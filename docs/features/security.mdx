...
- **Action needed**: something concrete to act on that Sencho can support right now, such as a
  ...
  Compose setting, a known-exploited (CISA KEV) CVE, or network-exposed affected images.
- **Monitoring**: Critical or High findings exist, but none currently have a concrete operator
  action Sencho can drive. Package fixes may exist without an applicable container-image update,
  or findings may already be triaged. Waiting for an upstream image and uncertain update
  availability appear in the review queue without a red masthead.
...
CVEs, detected secrets, unacknowledged Compose risks, and network-exposed affected images. When
only monitoring items remain (waiting for an upstream image, update availability unknown, network-exposed
images with no fix or known exploit, findings awaiting triage, stale or failed scans) the card is
titled **Review queue** and lists them with View findings shortcuts (and Check again when image
update availability could not be established and you can manage the node), so the operator can see
what to keep an eye on without a permanent alarm. Shortcuts that carry affected image identities
open Images already narrowed to those images, with a clearable banner so you can return to the full
list.
...
with host networking) carry a **Network exposed** evidence label on the row. That label means Compose
declares non-loopback reachability; it is not a claim that the service is reachable from the Internet.
Exposure intent comes from Networking (public, LAN, reverse proxy, and so on). Intentional
classification still counts as a risk factor for vulnerable images. Compose Doctor acknowledgements
are a separate triage surface and do not clear Security exposure posture. Selecting an
