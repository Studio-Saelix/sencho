...
- **Action needed**: something concrete to act on that Sencho can support right now, such as a
  ...
  Compose setting, a known-exploited (CISA KEV) CVE, or publicly exposed affected images.
...
CVEs, detected secrets, unacknowledged Compose risks, and publicly exposed affected images. When
only monitoring items remain (waiting for an upstream image, update availability unknown, exposed
...
images and shows a clearable banner with the reason and count. Images published on a public interface
also carry a **Publicly exposed** evidence label on the row. Selecting an
