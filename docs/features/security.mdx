...
- **Monitoring**: Critical or High findings exist, but none currently have a concrete operator
  ...
  findings may already be triaged, or workloads may be intentionally network-exposed without an
  independent Security driver. Waiting for an upstream image and uncertain update availability
  are review-level reasons (not Action needed), so they appear under **Why Monitoring** without a
  red masthead.
...
**Monitoring** means residual Critical/High risk remains visible and reviewable, while Security has
no confirmed operator action it can drive right now. That is not a claim of safety: raw detections,
package-fix evidence without a confirmed image update, waiting-upstream and update-check-uncertain
rows, and intentional exposure context stay on the page so residual risk is never silent.
