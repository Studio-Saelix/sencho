...
- A pulsing fuchsia dot flags that an image update is available and the latest check completed successfully.
- A muted warning icon appears when the last check was incomplete and an update was detected or previously recorded, but the full stack could not be verified. Incomplete results are not counted under **Updates**.
