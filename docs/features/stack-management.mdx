...
When a stack declares more than one Compose service, each service with a single matching container renders as one flat container card. That card carries **Update** or **Rebuild** (when eligible) immediately left of the image source links control, the update badge when a registry update is confirmed, and the **Service actions** kebab for Start / Stop / Restart.

Services with multiple replicas keep a shared header above their nested container cards. That header owns **Update** or **Rebuild**, the update badge, and **Start / Stop / Restart** for every replica. Nested cards keep logs, shell, ports, and metrics only.

A declared service with no running containers shows a compact row with the service name, Update/Rebuild when eligible, and the Service actions kebab (so you can still Start the service).
