...
Twenty-four further categories are written to notification history, and can be targeted by a mute rule, but are never dispatched to external channels or accepted by a routing rule:
...
- GitOps authority and rollout decisions: `gitops_source_accepted`, `gitops_placement_approved`, `gitops_rollout_authorized`, `gitops_rollout_paused`, `gitops_rollout_resumed`, `gitops_rollout_superseded`, `gitops_rollback_started`, `gitops_rollback_completed`, `gitops_rollback_partial_failed`, and `gitops_stateful_confirmation`.

The Git source, GitOps, and `network_auto_created` categories render as raw category strings in the bell because the frontend label map omits them, same as the `blueprint_*` categories above. The lifecycle markers carry their labels.
