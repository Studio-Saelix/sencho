...
<Note>
  Discord, Slack, and webhook URLs must use HTTPS. Apprise and ntfy endpoints may use HTTP or HTTPS. Every endpoint must parse as a URL. After save, Sencho shows an origin-only placeholder instead of the stored token. Paste a full URL again to rotate the destination or to use **Test**.
</Note>
...
The four `blueprint_*` categories are accepted by routing rules but are missing from the bell's category filter and the Settings category pickers because the frontend label map omits them.

Twenty-six further categories are written to notification history. They are never dispatched to external channels or accepted by a routing rule, and because a mute rule is applied when an alert is dispatched, a rule naming one of them is accepted by the API but does not hide the entry from the bell:

- Lifecycle markers: `drift_detected` (Drift detected), `drift_resolved` (Drift resolved), `update_started` (Update started), `health_gate_passed` (Health gate passed), `health_gate_failed` (Health gate failed), `rollback_generation_released` (Rollback protection released), and `network_auto_created`.
- Git source attempts: `git_pull_ready`, `git_plan_blocked`, `git_pull_unproven`, `git_pull_failed`, `git_apply`, `git_apply_failed`, `git_apply_rolled_back`, and `git_create`. An attempt that settles without proving a result is recorded as `git_pull_unproven`, separately from `git_pull_failed`, so a reconcile that is still in flight or was never able to establish an outcome is not reported as a failure.
- GitOps authority, rollout, confirmation, and health decisions: `gitops_source_accepted`, `gitops_placement_approved`, `gitops_rollout_authorized`, `gitops_rollout_paused`, `gitops_rollout_resumed`, `gitops_rollout_superseded`, `gitops_rollback_started`, `gitops_rollback_completed`, `gitops_rollback_partial_failed`, `gitops_stateful_confirmation`, and `gitops_health_failed`. A health verdict records an entry only when it is a confirmed failure, and only when the verdict is judged against the generation the target is being asked to run. A passing check, a check that could not reach a verdict, and a failure recorded against a generation that a rollout has already superseded are all silent.

The Git source, GitOps, and `network_auto_created` categories are missing from the bell's category filter and the Settings category pickers because the frontend label map omits them, same as the `blueprint_*` categories above. The lifecycle markers appear under their labels.
