...
Sencho ships Discord, Slack, and Generic Webhook. There is no built-in Email, MS Teams, Pushover, Telegram, Mattermost, Gotify, or Ntfy delivery. Use a Generic Webhook plus a small adapter, or feed the webhook into your existing alerting infrastructure.

There is no dispatch retry queue. If your endpoint is down at the moment Sencho tries to deliver, the row appears in the bell with `dispatch_error` set and is not redelivered.

The `Stacks` matcher in routing rules is exact-match against names you select from a combobox; there is no glob, regex, or label-pattern syntax in that field. Use **Labels** to express a wildcard-like grouping.
