...
<Note>
  ...
  **Virtual machines:** the memory tile, health verdict, and host RAM alerts account for hypervisor memory ballooning (TrueNAS/KVM, Proxmox). Ballooned memory is subtracted from used, so a VM with reclaimed guest memory does not read as memory-critical. See [VM memory ballooning](/getting-started/configuration#vm-memory-ballooning) for details.
</Note>
