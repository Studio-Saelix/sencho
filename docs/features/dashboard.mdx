...
<Note>
  ...
  **Nested LXC / Docker-in-LXC:** host CPU and memory totals come from the OS view inside the container. If that view is not virtualized (for example lxcfs is missing on a Proxmox LXC), the gauges can show the hypervisor host instead of the LXC. See [Nested LXC and Docker-in-LXC host metrics](/getting-started/configuration#nested-lxc-and-docker-in-lxc-host-metrics).
