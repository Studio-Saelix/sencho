...
A mono table of every stack discovered on the active node by default, sorted so the stacks demanding attention sit at the top. When your inventory has more than one node, a **This node | All nodes** control appears in the card header. **All nodes** fans out the same status, metrics, and GitOps lookups to every other reachable node and merges the results as each node answers, so a slow or denied node never blocks the rest.
