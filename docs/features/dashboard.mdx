...
Alert rules and severity routing are configured per stack in **Settings · Notifications**. See [Alerts and notifications](/features/alerts-notifications) for setup details and [Notification routing](/features/notification-routing) for per-category routing rules.
