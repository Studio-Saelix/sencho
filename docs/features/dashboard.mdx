...
<Note>
  ...
  **Nested hosts (LXC / Docker-in-LXC):** when the Sencho service joins the host cgroup namespace (`cgroup: host` in Compose, or `--cgroupns=host` for `docker run`), the CPU core count and memory total use the node's host-scoped cgroup limits, and host RAM alerts use the cache-aware cgroup working set. Without that setting, gauges keep the previous `/proc`-based behavior. See [Host cgroup capacity](/getting-started/configuration#host-cgroup-capacity-lxc-and-nested-docker).
