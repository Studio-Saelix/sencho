...
The phone dashboard uses the same eight-row collapse, shared state classifier, and composite navigation. When more than one node is available it shows the same **This node | All nodes** control; **Show all** / **Show less** replace the older jump away from Stack health.
