...
Federation actions require both an Admiral license and an admin user role. Viewers, operators, and lower-tier users do not see the cordon toggle on the node card or the Federation tab in the Fleet view. The `Cordoned` pill on the node card stays visible at every tier as a read-only signal, so non-Admiral operators can still understand why a node is skipping new work.
...
<AccordionGroup>
  ...
  <Accordion title="The Federation tab is not visible">
    Federation requires an Admiral license. On Skipper, Federation is hidden by design and the rest of the blueprint surface (catalog, deployments, drift) remains available. The `Cordoned` pill on a node card is visible at every tier; only the toggle and the Federation tab are gated.
  </Accordion>
