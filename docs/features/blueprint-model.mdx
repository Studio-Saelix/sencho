...
Each node in the scope must hold a recovery generation that names the selected application generation; a node that cannot prove that is reported as a failed target, and the rollout reads as a partially failed rollback rather than a completion.
