...
Enforce's auto-fix only ever redeploys the blueprint's *current* revision. Container-level drift (a stopped or crashed container) uses the ordinary deploy path. Frozen image-digest disagreement uses a digest-pinned overlay with `--pull never`, so authored tags on disk stay unchanged. For a remote target, Sencho freezes and repairs against that node's own rendered Compose model and Docker platform (fetched over the authenticated proxy), not the controlling instance's local filesystem. That repair also needs the target node to run a Sencho release that honors digest pins. A node that predates them would drop the pins and redeploy by tag, so the repair is refused and the node is reported as needing an upgrade instead, and the same applies when the node cannot be reached to confirm. The deployment row for that target ends in **Failed** with the reason, and a `blueprint_drift_correction_failed` notification fires. Upgrade the node, then trigger a deploy as you would for any other failed deployment; other drift modes are unaffected. A compose edit never reaches that auto-fix path on a stateful or state-unknown blueprint: any revision change re-enters **Awaiting confirmation** on every targeted node first, regardless of drift mode, so a human always signs off before a new revision touches existing data. See [Lifecycle: status transitions](#lifecycle-status-transitions).
