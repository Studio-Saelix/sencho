...
- a stateful deployment that reached a new node and is held for its first-deploy confirmation.

Each entry names the Blueprint and carries the GitOps operation it came from. These categories are history-only: they appear in the bell and can be muted, but they are not sent to your configured delivery channels.
