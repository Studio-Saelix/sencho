...
**Convert to Git** moves an existing live Git source onto this Blueprint. Pick the source, review the preview, and confirm. The Blueprint becomes Git-managed: Git is the content authority, the previous YAML is stored as a frozen snapshot, and rollout from that snapshot is blocked. Credentials stay with the Git source. Conversion also returns approval to pending.
...
<AccordionGroup>
  ...
  <Accordion title="Can I roll back to a previous revision?">
    Not through a one-click history. Blueprints intentionally do not keep a versioned revision history. To revert an Inline Blueprint, paste the prior compose into the editor and save; the reconciler treats the change as a new revision and redeploys. Git-managed Blueprints do not roll out from Git or from the stored snapshot. Detach Git first if you need Inline paste-and-apply.
  </Accordion>
  <Accordion title="Can a Git-managed Blueprint deploy?">
    No. Git-managed content cannot deploy from the stored snapshot. Convert and Adopt bind the live Git source and show that blocked state. Detach restores Inline editing when a snapshot exists.
  </Accordion>
