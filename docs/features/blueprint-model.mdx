...
- Manual **Evict** (including from an active row) requires a current approved remove outcome for that node. Confirm a rollout that removes the node first; Evict while the node is still desired, or without remove approval, returns a stale-guard error.
