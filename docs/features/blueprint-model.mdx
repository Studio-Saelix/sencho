...
Digest drift means a target runs an image digest other than the one in the approved artifact set. Sencho checks it on every reconciler tick, after the marker, revision, and container checks pass.

**What triggers it.** Sencho records the runtime image identity of each targeted node and compares it with the target's expected artifact set. When the expected set carries per-service digests, each service is compared. A digest drift is reported only when both sides are comparable: the approved set has exact or qualified evidence, and the observation is exact or qualified.

**What unverified means.** When the comparison cannot be proven, the result is unverified instead of drifted or matched. This happens when the node is unreachable, the runtime identity could not be collected, the observation is stale or comes from a locally built image, the observation has no per-service digests, or no approved artifact set exists. Unverified never triggers Enforce and never counts as converged. The **Drift** tab labels such a target **unverified** and states the reason.

**What Enforce does.** On a digest drift, Enforce redeploys with the approved artifact set pinned: a digest-pinned overlay with `--pull never`. It never pulls the latest tag, and authored tags on disk stay unchanged. If the approved digest cannot be resolved for the node's platform, the repair fails closed: nothing is deployed and a `blueprint_drift_correction_failed` notification fires. Expected and last-known-good pointers are never moved by a repair. After a successful repair the next observation reports the approved digest again.

Observe and Suggest report the same digest drift without repairing it. The **Drift** tab of the stack shows the expected and observed digest for each target. See [Drift Taxonomy](/features/drift-taxonomy) for how digest drift fits among the other drift classes.
