...
- **Authorize rollout** evaluates registry preflight, records the rollout authorization, and starts the sequential rollout across the approved targets. It appears while the rollout is not authorized or while the recorded authorization was granted against earlier inputs. A blocked preflight refuses the authorization with its reason; when a target cannot start, the authorization is recorded and the surface reports what stopped the rollout.

A disabled Blueprint withholds placement approval and rollout authorization until you enable it again, the same way **Apply now** waits for an Inline Blueprint to be enabled.
...
<AccordionGroup>
  ...
  <Accordion title="Can a Git-managed Blueprint deploy?">
    Yes, from an authorized accepted Git generation. Accept the source, approve the placement, then authorize the rollout; each step is its own action on the Blueprint sheet's GitOps section and on the application's page in GitOps. Place never uses the frozen detach snapshot on the Blueprint row. Convert and Adopt bind the live Git source; Detach restores Inline editing when a snapshot exists.
  </Accordion>
