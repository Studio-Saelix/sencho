...
2. **Approvals.** Fleet mutation is gated on three distinct approvals, not a single combined fingerprint:
   ...
   Inline Confirm Apply writes placement approval (and an Inline rollout generation). Git-managed place additionally requires source acceptance and rollout authorization for the accepted generation.
   Git-managed place also runs registry preflight before rollout authorization; a blocked result appears on the detail sheet until you fix registry readiness and place again.
...
<AccordionGroup>
  ...
  <Accordion title="Can a Git-managed Blueprint deploy?">
    Yes, from an authorized accepted Git generation. Place never uses the frozen detach snapshot on the Blueprint row. Convert and Adopt bind the live Git source; Detach restores Inline editing when a snapshot exists.
  </Accordion>
