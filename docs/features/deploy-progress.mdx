...
When the [health gate](/features/health-gated-updates) is observing the stack after a deploy or update, the modal shows the observation status and holds off the auto-close until the verdict lands. A passed gate resumes the countdown; a failed or unknown verdict keeps the modal open with the reason. Closing the modal never stops the observation.
