...
1. **Serve Sencho over HTTPS**, either behind a TLS reverse proxy (nginx, Traefik, Caddy) or with [native TLS on the instance](/getting-started/configuration#native-tls-on-the-control-instance), so session cookies travel on a confidential connection. See [Self-Hosting](/operations/self-hosting).
