...
1. **Serve Sencho over HTTPS**, either behind a TLS reverse proxy (nginx, Traefik, Caddy) or with [native TLS on the instance](/getting-started/configuration#native-tls-on-the-control-instance), so session cookies travel on a confidential connection. See [Self-Hosting](/operations/self-hosting). Compatible remotes also accept sealed registry credentials over plain HTTP; HTTPS and native TLS remain the recommended posture for session cookies and for remotes that have not yet upgraded to sealing.
