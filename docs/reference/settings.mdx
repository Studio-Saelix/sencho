...
Manage the Trivy scanner, scan policies, suppressions, and acknowledgements that power [vulnerability scanning](/features/vulnerability-scanning) across the fleet. The masthead publishes a **TRIVY** pill (`managed` / `host` / `missing`) and a **POLICIES** pill with the active rule count. This section is where security is *managed*; the dedicated [Security page](/features/security) is where it is *reviewed* (overview, findings, history, and the scanner setup tab, which surfaces the same Trivy controls).
