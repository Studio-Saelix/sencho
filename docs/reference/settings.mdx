...
| Pill | Meaning |
|------|---------|
| **SCOPE** `operator` / `account` / `browser` / `global` | Setting applies to your account (`operator`), to your saved interface preferences, synced across your devices (`account`, for Appearance), to this browser only (`browser`, for browser-local workflow toggles), or to the whole instance (`global`, every other non-node group) |
...
**Scope:** Account (preferences are saved to your account and synced to every device you sign in from)

Control how Sencho looks and how dense the workspace feels. Your choices follow your account: sign in on another browser or device and the same look is already there. See [Appearance](/features/appearance) for the full walkthrough.
...
| **Sidebar layout** | **Fixed** (default) keeps the desktop stacks sidebar at its standard width. **Resizable** makes the sidebar/workspace divider draggable and keyboard-adjustable (248 to 440 px, saved on release or per arrow-key press). **Reset sidebar layout** returns it to Fixed and the default width. Phone layouts are unchanged. |
| **Stack detail layout** | **Fixed** (default) preserves the even split between stack status and the Anatomy panel. **Resizable** keeps both panes at least 320 px wide and supports an Anatomy preference up to the 4096 px safety ceiling. Width is saved on release or per arrow-key press. **Reset Anatomy panel layout** restores Fixed without changing the sidebar. The compose editor and phone layout are unchanged. |
...
| Action | Description |
|--------|-------------|
| **Reset interface preferences** | Restores every appearance and navigation preference to their defaults for your account and clears the browser-local editor toggles, then reloads the page. Use this if a display setting wedges the layout. |
...
When enabled, clicking **Save & Deploy**, **Save Only**, or **Save & Pull Images** in the compose or env editor opens a side-by-side diff modal before writing anything to disk. The left pane shows the current on-disk content; the right pane shows your unsaved edits, with additions highlighted green and removals highlighted red.
