...
| **Stack detail layout** | **Fixed** (default) preserves the even split between stack status and the Anatomy panel. **Resizable** makes the Anatomy divider draggable and keyboard-adjustable (320 to 960 px, saved on release or per arrow-key press). **Reset Anatomy panel layout** restores Fixed without changing the sidebar. The compose editor and phone layout are unchanged. |
