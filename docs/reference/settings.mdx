...
<Note>
  Creating, editing, and deleting suppression rules is admin-only.
</Note>

**Scope:** Global, admin-only

Create rules that mute or drop matching alerts from the bell, external channels, or both. Suppression is evaluated before routing. The masthead publishes **RULES** and **ACTIVE** counts.
...
See [Suppression rules](/features/alerts-notifications#suppression-rules) for the full walkthrough and bell shortcuts.
