...
export function computeMeshMembershipImpact(
    ...
    return { direct, others, otherNodeCount: new Set(others.map((o) => o.nodeName)).size };
}
...
export function describeMeshMembershipImpact(impact: MeshMembershipImpact, change: MeshMembershipChange): string {
    ...
    return `${first} ${plural(impact.others.length, 'other meshed stack', 'other meshed stacks')} on ${plural(impact.otherNodeCount, 'node', 'nodes')} also ${verb} to refresh their hostnames, so expect brief connection drops.`;
}
