...
export function MeshRouteDetailSheet({ open, onOpenChange, alias, canManage, status, aliases, onChanged }: Props) {
    ...
    return (
                            ...
                            <div className="grid grid-cols-2 gap-x-4 gap-y-1.5 text-xs">
                                ...
                                <div className="flex items-center gap-1.5 min-w-0">
                                    ...
                                    <button
                                        ...
                                        className="shrink-0 text-stat-subtitle hover:text-brand transition-colors"
        ...
        <ConfirmModal
            ...
            description={
                target
                    ? `This removes ${stackAliasCount} ${stackAliasCount === 1 ? 'hostname' : 'hostnames'} published by ${target.stack}. ${describeMeshMembershipImpact(
                        computeMeshMembershipImpact(status, { kind: 'opt-out', nodeId: target.nodeId, stackName: target.stack }),
                        { kind: 'opt-out', nodeId: target.nodeId, stackName: target.stack },
                    )}`
                    : undefined
            }
